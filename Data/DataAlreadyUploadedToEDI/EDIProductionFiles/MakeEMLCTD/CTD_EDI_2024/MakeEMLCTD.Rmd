...
```{r}
ctd <- read.csv("CTD_2013_2024.csv")
ctd_fix <- ctd %>%
  mutate(
...
write.csv(ctd_fix, "CTD_2013_2024.csv", row.names = FALSE)

ctd_maintlog <- read_csv("https://raw.githubusercontent.com/CareyLabVT/Reservoirs/62d78f19fc8a8520da6f9c81619a2d651c569f97/Data/DataNotYetUploadedToEDI/Raw_CTD/CTD_Maintenance_Log.csv")

write.csv(ctd_maintlog, "ctd_Maintenance_Log.csv", row.names = F)
```
...
make_eml(path = getwd(),
         ...
         data.table.name = c("CTD dataset 2013-2024",
                             "Descriptions of sample sites",
                             "Maintenance log 2013 - 2024"),
         ...
         other.entity.description = c('High-level QAQC script used to process data from 2018-2024: references scripts in helper_scripts.zip', 
                                      ...
                                      as referenced by CTD_QAQC_2018_2024.R"),
         temporal.coverage = c("2013-03-07", "2024-12-31"),
         maintenance.description = "ongoing",
         user.domain = "EDI",
         user.id = "ccarey",
         package.id = "edi.1113.8")
