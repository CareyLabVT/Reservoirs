# 2024 QAQC for TNTP, DOC, lachat solubles
#performs QAQC for 2024 chemistry variables measured in the Stream Team analytical lab
...
####get datetimes df together

#CCR datetimes; this is only needed for DWH's 2024 CCR samples, can delete in 2025
ccr_datetimes <- read.csv("./Data/DataNotYetUploadedToEDI/NutrientData/collation/2024/CCR_datetimes.csv") 

ccr_datetimes <- ccr_datetimes |> 
  filter(Rep == 1) |> #filter to rep 1 since collected at same time as 2
...
#datetimes df
datetimes <- rbind(ltreb_datetimes, ccr_datetimes) |> 
  mutate(Time = as.character(Time))

#### Read in compiled data and add time to datetime
chem2024_raw <- read.csv("./Data/DataNotYetUploadedToEDI/NutrientData/collation/2024/chem2024_forQAQC.csv") |> 
  mutate(Date = mdy(Date),
         Site = as.character(Site))

#make datetime column and order for QAQC
chem2024_forQAQC <- left_join(chem2024_raw, datetimes, by = c("Reservoir", "Site", "Date", "Depth_m")) |> 
  ...
  mutate(Lab_Rep = data.table::rowid(Reservoir, Site, DateTime, Depth_m, Rep)) |> #make rep column to identify dups
  select(Reservoir, Site, Date, Time, DateTime, Depth_m, Rep, Lab_Rep, everything()) #order data 



#### TNTP 
TNTP <- chem2024_forQAQC |> 
  select(Reservoir, Site, DateTime, Depth_m, Rep, Lab_Rep, TP_ugL, TN_ugL, Flag_DateTime)

#make sure all analytes are numeric
TNTP$TN_ugL <- as.numeric(TNTP$TN_ugL)
TNTP$TP_ugL <- as.numeric(TNTP$TP_ugL)
...
#order TNTP df
TNTP <- TNTP %>% arrange(Reservoir, DateTime, Site, Depth_m, Rep, Lab_Rep)

# look for duplicate values and average them, while adding flag = 7
#THIS is only for dups on the instrument, not averagin field dups (ie. the Rep column)
TNTP_dups <- duplicated(TNTP[,1:5]) 
table(TNTP_dups)['TRUE']

#check if dups are just ISCOs
TNTP_noISCO <- TNTP |> filter(Site != 100.1)
TNTP_dups_noISCO <- duplicated(TNTP_noISCO[,1:5]) 
...
for (i in 1:length(TNTP_dups)) {
  if(TNTP_dups[i]=='TRUE'){
    ifelse(TNTP$Lab_Rep[i]=="2",TNTP$TP_ugL_AVG[i]<- mean(c(TNTP$TP_ugL[i], TNTP$TP_ugL[i-1])), TNTP$TP_ugL_AVG[i])
    ifelse(TNTP$Lab_Rep[i]=="2",TNTP$TN_ugL_AVG[i]<- mean(c(TNTP$TN_ugL[i], TNTP$TN_ugL[i-1])), TNTP$TN_ugL_AVG[i])
    # assign this to the other duplicate as well
    ifelse(TNTP$Lab_Rep[i]=="2",TNTP$TP_ugL_AVG[i-1]<- mean(c(TNTP$TP_ugL[i], TNTP$TP_ugL[i-1])), TNTP$TP_ugL_AVG[i-1])
    ifelse(TNTP$Lab_Rep[i]=="2",TNTP$TN_ugL_AVG[i-1]<- mean(c(TNTP$TN_ugL[i], TNTP$TN_ugL[i-1])), TNTP$TN_ugL_AVG[i-1])

    # flag as 7, average of two reps
    ifelse(TNTP$Lab_Rep[i]=="2", TNTP$Flag_TP[i] <- 7, TNTP$Flag_TP[i])
    ifelse(TNTP$Lab_Rep[i]=="2",TNTP$Flag_TN[i] <- 7, TNTP$Flag_TN[i])
    ifelse(TNTP$Lab_Rep[i]=="2",TNTP$Flag_TP[i-1] <- 7, TNTP$Flag_TP[i-1])
    ifelse(TNTP$Lab_Rep[i]=="2",TNTP$Flag_TN[i-1] <- 7, TNTP$Flag_TN[i-1])
  }
}

# remove dups (5 in 2024 from ISCOs)
TNTP <- TNTP[!TNTP_dups,]
...
#                     5.7      104.1                        #
############################################################  
## MDL's come from MDL rolling spike blanks
...
for (i in 1:nrow(TNTP)) {
  ifelse(TNTP$TP_ugL[i] < 5.7 & TNTP$Flag_TP[i]==7,
    TNTP$Flag_TP[i] <- "73",
  ifelse(TNTP$TP_ugL[i] < 5.7,
    TNTP$Flag_TP[i] <- 3, TNTP$Flag_TP[i]))
  }


for (i in 1:nrow(TNTP)) {
  ifelse(TNTP$TN_ugL[i] < 104.1 & TNTP$Flag_TN[i]==7,
    TNTP$Flag_TN[i] <- "73",
  ifelse(TNTP$TN_ugL[i] < 104.1,
...
doc <- chem2024_forQAQC |> 
  select(Reservoir, Site, DateTime, Depth_m, Rep, Lab_Rep, DIC_mgL, DC_mgL, DOC_mgL, DN_mgL, Flag_DateTime)
...
#order doc df
doc <- doc %>% arrange(Reservoir, DateTime, Site, Depth_m, Rep, Lab_Rep)
...
for (i in 1:nrow(doc)) {
  if(doc$DC_mgL[i] < 0 & !is.na(doc$DC_mgL[i])){   #added is.na part because of NAs introduced in NPOC data 
    doc$DC_mgL[i] <- 0
    if(doc$Flag_DC[i]>0){
...
for (i in 1:nrow(doc)) {
  if(doc$DN_mgL[i] <0 & !is.na(doc$DN_mgL[i])){  #added is.na so loop runs; but some NAs set on days with bad calibration curve
    doc$DN_mgL[i] <- 0
    if(doc$Flag_DN[i]>0){
...
# clean up DOC data
# look for duplicate values and average them, while adding flag = 7
#only averaging lab dups
doc_dups <- duplicated(doc[,1:5],fromLast = FALSE)
table(doc_dups)['TRUE']
...
#                 see rolling spike blank tab in MDL sheet      #
#                 if below detection, flag = 3                  #
#     2024 MDLS (in mg/L) from 'rolling spiked blank' tab:      # 
#                    DIC     DOC     DC    DN                   #
#                    0.95   0.94   1.37   0.08                  #
#################################################################
#    Historical MDL's:                                     #
...
#    2024: DIC = 0.95; DOC = 0.94; DC = 1.37; DN = 0.08    #
############################################################

# DIC
for (i in 1:nrow(doc)) {
  if(doc$DIC_mgL[i] <0.95 & !is.na(doc$DIC_mgL[i])){
    if(doc$Flag_DIC[i]>0){
...
# DC
for (i in 1:nrow(doc)) {
  if(doc$DC_mgL[i] < 1.37 & !is.na(doc$DC_mgL[i])){
    if(doc$Flag_DC[i]>0){
...
# DN
for (i in 1:nrow(doc)) {
  if(doc$DN_mgL[i] <0.08 & !is.na(doc$DN_mgL[i])){
    if(doc$Flag_DN[i]>0){
...
#add NPOC flag 
for (i in 1:nrow(doc)) {
  if(is.na(doc$DIC_mgL[i]) & is.na(doc$DC_mgL[i])  ){  #if both DIC and DC are NA then it was an NPOC run
    if(doc$Flag_DOC[i]>0){
      doc$Flag_DOC[i] <- paste0(doc$Flag_DOC[i], 8)
      
    }
    else{doc$Flag_DOC[i] <- 8}
  }
}
...
np <- chem2024_forQAQC |> 
  select(Reservoir, Site, DateTime, Depth_m, Rep, Lab_Rep, NO3NO2_ugL, NH4_ugL, PO4_ugL, Flag_DateTime)
...
#order np df
np <- np %>% arrange(Reservoir, DateTime, Site, Depth_m, Rep, Lab_Rep)
...
#only average lab reps not field dups
np_dups <- duplicated(np[,1:5]) 
table(np_dups)['TRUE']
...
np_nodups$NO3NO2_ugL <- as.numeric(np_nodups$NO3NO2_ugL)
np_nodups$NH4_ugL <- as.numeric(np_nodups$NH4_ugL)
np_nodups$PO4_ugL <- as.numeric(np_nodups$PO4_ugL)
...
##################################################################
#    2024 field season average: if below detection, flag as 3    #
#    using the following rollin spike blank from MDL sheet       #          
#                      NH4   PO4   NO3                           # 
#                      4.7   4.5   5.2                           #                        
##################################################################
#    Historical MDL's:                        #
...
for (i in 1:nrow(np)) {
  if(np$NO3NO2_ugL[i] < 5.2  & !is.na(np$NO3NO2_ugL[i])){  #add is.na for NO3 point that was removed for bad peak integration
    if(np$Flag_NO3NO2[i]>0){
...
chem <- chem %>%
  select(c(-Flag_DateTime.y, -Flag_DateTime.x, -Lab_Rep, -Lab_Rep.x, -Lab_Rep.y)) %>% 
  mutate(TP_ugL = as.numeric(TP_ugL),
         TN_ugL = as.numeric(TN_ugL),
...
## Set flag to 1 for variables with missing data; largely for field reps that didn't get run across all instruments
#TN
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$TN_ugL[i])){
    chem_final$Flag_TN_ugL[i] <- 1
  }
}

#TP
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$TP_ugL[i])){
    chem_final$Flag_TP_ugL[i] <- 1
  }
}

#NH4
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$NH4_ugL[i])){
    chem_final$Flag_NH4_ugL[i] <- 1
  }
}

#SRP
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$SRP_ugL[i])){
    chem_final$Flag_SRP_ugL[i] <- 1
  }
}

#NO3NO2
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$NO3NO2_ugL[i])){
    chem_final$Flag_NO3NO2_ugL[i] <- 1
  }
}

#DOC
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$DOC_mgL[i])){
    chem_final$Flag_DOC_mgL[i] <- 1
  }
}

#DIC
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$DIC_mgL[i])){
    chem_final$Flag_DIC_mgL[i] <- 1
  }
}

#DC
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$DC_mgL[i])){
    chem_final$Flag_DC_mgL[i] <- 1
  }
}

#DN
for (i in 1:nrow(chem_final)) {
  if(is.na(chem_final$DN_mgL[i])){
    chem_final$Flag_DN_mgL[i] <- 1
  }
}


########## Order final data set and write csvs #############################


#order chem
chem_final<- chem_final %>% arrange(Reservoir, DateTime, Site, Depth_m, Rep)

#remove NA rows
chem_final <- chem_final[!is.na(chem_final$Reservoir),]

#Save final DF with ISCO and SUNP
write.csv(chem_final, "./Data/DataAlreadyUploadedToEDI/EDIProductionFiles/MakeEMLChemistry/2024/2024_chemistry_collation_with_ISCO_SUNP.csv", row.names = F)
...
#make sites numberic again with SUNP gone
chem_final$Site <- as.numeric(chem_final$Site)

#save final df
write.csv(chem_final, "./Data/DataAlreadyUploadedToEDI/EDIProductionFiles/MakeEMLChemistry/2024/2024_chemistry_collation_final_nocommas.csv", row.names = F)
