...
#Replace this link with the new EDI pasta

inUrl1  <- "https://pasta-s.lternet.edu/package/data/eml/edi/52/42/d37e703eee6c7c7e28982f63703a0721" 
infile1 <- tempfile()
...
```
**Time Series for entire data set - Phaeophytin**
```{r Time Series for entire data set Phaeophytin}
# Facetted plot of the datapoints within each reservoir over the entire dataset 
ggplot(subset(all_chla), aes(x = DateTime, y = Pheo_ugL, col = Reservoir)) +
  geom_point(size = 1) + 
  facet_grid(Reservoir ~., scales = "free_y") + 
  ggtitle("Entire Dataset Timeseries - Phaeophytin")
```
...
#check the flag column
phaeo_below0 <- subset(all_chla, Pheo_ugL < 0)
phaeo_below0
...
```
**All Sites, All Years, Monthly Mean - Phaeophytin**
```{r All Sites, All Years, Monthly Average - Phaeophytin}
# Facetted plot of the range of each reservoir for each year and the mean of the range
ggplot(subset(chla_YMD), aes(x = Month, y = Pheo_ugL, col = Reservoir))+ 
  ...
  scale_y_continuous("Concentration (ugL)") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = 'none') + 
  ggtitle("Range and Mean of Range - Phaeophytin")
```
...
```
**All Sites, All Years, Monthly Median - Phaeophytin**
```{r All Sites, All Years, Monthly Median - Phaeophytin}
# Facetted plot of the range of each reservoir for each year and the median of the range
ggplot(subset(chla_YMD, Site == 50), aes(x = Month, y = Pheo_ugL, col = Reservoir))+ 
  ...
  scale_y_continuous("Concentration (ugL)") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = 'none') + 
  ggtitle("Range and Median of Range - Pheophytin")
```
...
2025 - BVR has a very high value on Aug. 4 2025 - KKH ran this sample, it was definitely that high. 
...
```{r All Sites, All Years, Monthly Median, samples colored by depth - Phaeophytin}
# Facetted plot of the range of each reservoir for each year and the median of the range with depth
ggplot(subset(chla_YMD, Site == 50), aes(x = Month, y = Pheo_ugL, col = Depth_group))+ 
  ...
  scale_y_continuous("Concentration (ugL)") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = 'left') + 
  ggtitle("Phaeophytin - Median, by depth")
```
**Flags**
...
Flag_Chla_ugL 6     Ratio of concentration before acid to conc after acid addition was greater than 1.72
Flag_Chla_ugL 7     High turbidity values

```{r Subset to Current Year Only}
...
ggplot(subset(chla_current), aes(x = DOY, y = Chla_ugL, pch = as.factor(Flag_Chla_ugL), color = as.factor(Flag_Chla_ugL))) +
  geom_point(size = 1) + 
  facet_grid(Reservoir ~., scales = "free_y") + 
...
```{r Subset to Current Year Only (2023) - Pheophytin}
#subset data to FCR and BVR
chla_current <- chla_YMD %>% 
  subset((Year=="2024")) %>% 
   mutate(DOY = yday(DateTime)) 
...
ggplot(subset(chla_YMD, Year == 2025), aes(x = DateTime, y = Chla_ugL, col = Depth_m)) +
  geom_point(size = 1) + 
  facet_grid(Reservoir ~., scales = "free_y") + 
  ggtitle("Timeseries - Chlorophyll a")
```
**Time Series for entire data set - Phaeophytin**
```{r Time Series for entire data set - Phaeophytin}
# Facetted plot of the datapoints within each reservoir over the entire dataset 
ggplot(subset(chla_YMD, Year == 2025), aes(x = DateTime, y = Pheo_ugL, col = Depth_m)) +
  geom_point(size = 1) + 
  facet_grid(Reservoir ~., scales = "free_y") + 
  ggtitle("This year's timeseries - Pheophytin")
```
...
# Variable_startyear_endyear.csv
 write.csv(all_chla, "filt-chla_2014_2025.csv", row.names = F)
```
