---
title: "FluoroProbe_inspection_2014_2025"
...
pacman::p_load(tidyverse, lubridate, akima, reshape2, 
               ...
               devtools, EDIutils, xml2, httr, here)
...
# package_ID <- 'edi.272.8' #edi.1304.1 <- this is the one currently in staging. https://portal-s.edirepository.org/nis/mapbrowse?scope=edi&identifier=1304&revision=1
...
example_file_for_colnames <- here("Data/DataAlreadyUploadedToEDI/CollatedDataForEDI/FluoroProbeData/20140404_CCR_50.txt")
...
historic_data_2017<- here("Data/DataAlreadyUploadedToEDI/CollatedDataForEDI/FluoroProbeData/FP_2017_data/FP_recal_2017.txt")
...
maintenance_file <- here("Data/DataNotYetUploadedToEDI/FluoroProbe/fluoroprobe_maintenance.csv")
out_file <- NULL
start_date <- as.Date("2014-05-03")
...
write.csv(current_df, "./fluoroprobe_2014_2025.csv", row.names = FALSE)
```
...
current_df <- read_csv('https://pasta-s.lternet.edu/package/data/eml/edi/1764/3/09255be1f3f7cb4f67064407aa21e2fc') %>%
  mutate(DateTime = force_tz(DateTime, tzone = "America/New_York"))

# current_df <- read_csv(here("/Data/DataAlreadyUploadedToEDI/EDIProductionFiles/MakeEMLFluoroProbe/2025/fluoroprobe_2014_2025.csv")) %>%
#    mutate(DateTime = force_tz(DateTime, tzone = "America/New_York"))
...
Flags
0: all good!
1: data recalibrated post inspection
2: instrument malfunction
3: negative transmission value
4: data suspect due to poor calibration
5: negative RFU value
...
for (i in 1:length(unique(current_df$CastID))){ #for every unique FP cast
  ...
  #maria added this too 
  safe_datetime <- format(as.POSIXct(profile$DateTime[1]),
                        "%Y-%m-%d_%H-%M-%S")
castname <- paste(
  profile$Reservoir[1],
  profile$Site[1],
...
# remove anything else sketchy just in case
castname <- str_replace_all(castname, "[^A-Za-z0-9_-]", "")
  ...
  #filename = paste0("C:/Users/maria/OneDrive/2023 Rtudio/EDI_cast_review/profiles/",castname,".png")
  #ggsave(filename = filename, plot = profile_plot, device = "png")
  
  #Maria commented out above and added this to address saving issue
  outpath = "C:/Users/maria/OneDrive/2023 Rtudio/EDI_cast_review/profiles/"
  ggsave(
  filename = file.path(outpath, paste0(castname, ".png")),
  plot = profile_plot,
  width = 6,
  height = 8,
  dpi = 300
...
####maria edit ends####^^^^
  ...
  # filename = paste0("C:/Users/maria/OneDrive/2023 Rtudio/EDI_cast_review/RFU/",castname,".png")
  # ggsave(filename = filename, plot = profile_plot, device = "png")
   #Maria commented out above and added this to address saving issue
  outpath = "C:/Users/maria/OneDrive/2023 Rtudio/EDI_cast_review/RFU/"
  ggsave(
  filename = file.path(outpath, paste0(castname, ".png")),
  plot = profile_plot,
  width = 6,
...
####maria edit ends####^^^^
  ...
  profile_plot <- ggplot(data = profile, aes(x = Temp_C, y = Depth_m))+
    ...
    ggtitle(castname)+
    theme_bw()
  # filename = paste0("C:/Users/maria/OneDrive/2023 Rtudio/EDI_cast_review/temperature",castname,".png")
  # ggsave(filename = filename, plot = profile_plot, device = "png")
   #Maria commented out above and added this to address saving issue
  outpath = "C:/Users/maria/OneDrive/2023 Rtudio/EDI_cast_review/temperature/"
  ggsave(
  filename = file.path(outpath, paste0(castname, ".png")),
  plot = profile_plot)
...
####maria edit ends####^^^^
  ...
  profile_plot <- ggplot(data = profile, aes(x = Transmission_perc, y = Depth_m))+
    ...
    ggtitle(castname)+
    theme_bw()
  # filename = paste0("C:/Users/maria/OneDrive/2023 Rtudio/EDI_cast_review/transmission/",castname,".png")
  # ggsave(filename = filename, plot = profile_plot, device = "png")
 
  #Maria commented out above and added this to address saving issue
  outpath = "C:/Users/maria/OneDrive/2023 Rtudio/EDI_cast_review/transmission/"
  ggsave(
  filename = file.path(outpath, paste0(castname, ".png")),
  plot = profile_plot)
...
## REVIEWER CAN START HERE AGAIN- Define heatmap function
...
flora_heatmap(fp_data = current_df, reservoir = "FCR", year = 2025, site = 50, z = "TotalConc_ugL")
flora_heatmap(fp_data = current_df, reservoir = "FCR", year = 2025, site = 40, z = "TotalConc_ugL")
flora_heatmap(fp_data = current_df, reservoir = "FCR", year = 2025, site = 30, z = "TotalConc_ugL")
flora_heatmap(fp_data = current_df, reservoir = "FCR", year = 2025, site = 20, z = "TotalConc_ugL")
flora_heatmap(fp_data = current_df, reservoir = "FCR", year = 2025, site = 10, z = "TotalConc_ugL")
flora_heatmap(fp_data = current_df, reservoir = "CCR", year = 2025, site = 50, z = "TotalConc_ugL")
flora_heatmap(fp_data = current_df, reservoir = "BVR", year = 2025, site = 50, z = "TotalConc_ugL")
```
...
##Visualize individual casts for 2025
```{r, echo=FALSE, message=FALSE, warning=FALSE}
PLOT_ALL_CASTS = TRUE
#create png plots for every cast for QAQC purposes (algal biomass)
annualcasts <- current_df|>
  filter(year(DateTime) == 2025) #change this if you would like to see another year
  #filter(Reservoir == "XXX") #add this if you would like to just see one reservoir

if(PLOT_ALL_CASTS == TRUE){

for (i in 1:length(unique(annualcasts$CastID))){ #for every unique FP cast
  profile = subset(annualcasts, CastID == unique(annualcasts$CastID)[i])
  # castname = paste(profile$Reservoir[1], profile$Site[1], profile$DateTime[1],
  #                  sep = "-")
...
Negative RFU values are likely an artifact of quenching. QAQC is applied to the concentration data. 
An ok range for negative numbers in RFUs is probably between 0 and -20. Keep an eye on this, but negative numbers are ok. 
```{r}
current_df |>
...
current_df |>
  ...
  pivot_longer(RFU_370nm:RFU_610nm, values_to = 'RFU', names_to = 'wavelength') |> 
  filter(RFU < 0) |> 
  ggplot(aes(x=DateTime, y= RFU, colour = as_factor(wavelength))) + 
  facet_wrap(Reservoir~Site) + 
  geom_point()
...
#is this updated in the right place?
 maint <- read_csv('https://raw.githubusercontent.com/CareyLabVT/Reservoirs/refs/heads/master/Data/DataNotYetUploadedToEDI/FluoroProbe/fluoroprobe_maintenance.csv')
...
```
#### Check that the columns have flags 
