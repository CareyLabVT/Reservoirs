---
title: "YSI PAR inspection 2013_2025"
...
pacman::p_load(tidyverse, lubridate, gsheet, plotly, htmltools)
#current time of QAQC for graphing
current_time_start=ymd_hms("2025-01-01 00:01:00", tz = "America/New_York")
...
current_df <- ysi_qaqc(data_file = 'https://docs.google.com/spreadsheets/d/1MX__IelyQBHO1bNxAltfYT_r_pJisMuiMtDG4oxkOok',
            gsheet_data = TRUE,
            maintenance_file = "https://raw.githubusercontent.com/CareyLabVT/Reservoirs/master/Data/DataNotYetUploadedToEDI/YSI_PAR/m#aintenance_log.csv",
            outfile = NULL,
...
# MAKE SURE TO UPDATE THE PASTA FROM THE VERSION YOU WANT
                                                                        ### CHANGE THIS BELOW 
                                                                              #########   
 #current_df <-read_csv("https://pasta-s.lternet.edu/package/data/eml/edi/1140/5/514d382eccf21428985abbd40e1d7d50")
...
## Identify which sites had measurements during the specified timeframe
Make sure to include plots for any sites that data was collected for this year for better viewing.

```{r ID active sites}
# Summarize sites and count distinct days
sampled_sites <- profiles_long %>%
  filter(
    DateTime >= current_time_start,
    DateTime <= current_time_end,
    !is.na(value)  # only rows with actual measurements
  ) %>%
...
###Current year all parameters at Beaverdam active sites
```{r current BVR, echo=FALSE}
res <- "BVR"
active_site_names <- sampled_sites %>%
  filter(Reservoir == res) %>%
  pull(Site)

# Parameters (all metrics between Temp_C and pH)
param <- current_df %>%
  select(which(names(.) == "Temp_C"):which(names(.) == "pH")) %>%
  colnames()

# Build a list to store all plots
all_plots <- list()

for (site in active_site_names) {
  
  # Filter and split by metric
  a <- profiles_long %>%
    filter(
      Reservoir == res,
      Site == site,
    ...
    (\(x) split(x, x$metric))() %>%
    lapply(function(df) {
      ggplot(df, aes(DateTime, Depth_m, col = value)) +
        geom_point(size = 2) +
        scale_color_viridis_c(name = unique(df$metric)) +
        scale_x_datetime("Date", date_breaks = "1 month", date_labels = "%b %Y") +
        scale_y_reverse("depth(m)") +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
        ggtitle(paste(res, site, unique(df$metric), sep = "_"))
    })
  
  # Append plots for this site to the main list
  all_plots <- c(all_plots, a)
}

# Display all plots in R Markdown
...
###Current year all parameters at active FCR Sites
```{r}
res <- "FCR"
active_site_names <- sampled_sites %>%
  filter(Reservoir == res) %>%
...
###Current year all parameters at CCR active sites
...
write_csv(current_df, 'handheld-sensors_2013_2025.csv')
#list.files()
```

```{r Save maintlog and function, eval=FALSE, include=FALSE}
# Maintenance Log
download.file("https://raw.githubusercontent.com/CareyLabVT/Reservoirs/master/Data/DataNotYetUploadedToEDI/YSI_PAR/maintenance_log.csv", "handheld-sensors_maintenancelog_2013_2025.csv")
# qaqc function
download.file("https://raw.githubusercontent.com/CareyLabVT/Reservoirs/master/Scripts/L1_functions/ysi_create.R", "handheld-sensors_qaqc_2013_2025.R")
```
