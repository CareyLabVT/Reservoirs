...
raw_profiles <- dir(path = "~/Dropbox/EDI_day/", pattern = "*_profiles.csv") %>% 
  map_df(~ read_csv(file.path(path = "~/Dropbox/EDI_day/", .), col_types = cols(.default = "c")))
str(raw_profiles)




raw_profiles <- filter(raw_profiles, is.na(Reservoir) == FALSE)

#date format
raw_profiles$DateTime <- ymd_hms(raw_profiles$DateTime)
str(raw_profiles)
...
profiles <- raw_profiles %>%
  ...
  # Add 'flag' columns for each variable; 1 = flag for NA value
   mutate(Flag_pH = ifelse(is.na(pH), 1, 0),
         ...
         Flag_DO = ifelse(is.na(DO_mgL), 1,
                          ifelse(DO_mgL > 35, 2, 0)),
         Flag_DOSat = ifelse(is.na(DOSat), 1, 0), # Flag 2 = inst. malfunction
  ...
  select(Reservoir, Site, DateTime, Depth_m, Temp_C, DO_mgL, DOSat, 
         ...
         Flag_Cond, Flag_PAR, Flag_ORP, Flag_pH) %>%
  arrange(Reservoir, DateTime, Depth_m) 

#Get rid of rows that are all NAs
profiles <- filter(profiles, is.na(Reservoir) == FALSE)
profiles <- profiles[!duplicated(profiles),]
which(duplicated(profiles))
# Write to CSV (using write.csv for now; want ISO format embedded?)
write.csv(profiles, '~/Dropbox/EDI_day/YSI_PAR_profiles_2013-2020.csv', row.names=F)
...
# Plot all values
ggplot(subset(profiles_long,year==2020), aes(x = DateTime, y = value, col=Reservoir)) +
  geom_point(size=1) +
...
ggplot(subset(profiles_long, Site=="50" & year==2020), aes(x = DateTime, y = value, col=Depth_m)) +
  geom_point(cex=2) +
...
ggplot(subset(profiles_long, Reservoir=='FCR' & year==2020), aes(x = DateTime, y = value, col=Depth_m)) +
  geom_point(cex=2) +
