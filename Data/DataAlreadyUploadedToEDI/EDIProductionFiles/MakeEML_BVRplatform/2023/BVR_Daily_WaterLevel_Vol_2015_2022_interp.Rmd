...
This script is used to create BVR_Daily_WaterLevel_Vol_2015_2022_interp.csv, which contains daily water level and volume associated with each date from 7 July 2015 - 4 May 2022 in Beaverdam Reservoir. 
This script uses EDI-published BVR sensor data from 2020-2022 to calculate daily water level using the pressure sensor, and water level observations from both the Western Virginia Water Authority and the Reservoir Group field crew from 2015-2020 (before high-frequency sensors were deployed) to interpolate water level and obtain daily values. We then use bathymetry data from EDI to associate a volume to the variable water level observations for each day.


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
pacman::p_load(RCurl, tidyverse, lubridate, gsheet)
...
bv_wl_19 <- gsheet::gsheet2tbl("https://docs.google.com/spreadsheets/d/1XLJu5T7qsWtbQDBDEv3_63yfNB_GVXgs/edit#gid=353013140") |> 
  mutate(WaterLevel_ft = as.numeric(WaterLevel_ft),
         DateTime_temp = as.POSIXct(DateTime, format="%m/%d/%y %H:%M", tz="EST"),
         DateTime_temp2 = as.POSIXct(paste0(DateTime,"12:00:00"), format="%m/%d/%Y %H:%M:%S", tz="EST"),
         DateTime = ifelse(is.na(DateTime_temp), DateTime_temp2, DateTime_temp),
         DateTime = as.POSIXct(DateTime, format="%m/%d/%Y %H:%M:%S", tz="EST")) |> 
  select(-c(DateTime_temp, DateTime_temp2))
           
bv_wl_20 <- gsheet::gsheet2tbl("https://docs.google.com/spreadsheets/d/1AsFAufJM2k1u2CQwpb3tu6S6ts8DNG6C/edit#gid=1740372423") |> 
    mutate(WaterLevel_ft = as.numeric(WaterLevel_ft),
         DateTime_temp = as.POSIXct(DateTime, format="%m/%d/%y %H:%M", tz="EST"),
         DateTime_temp2 = as.POSIXct(paste0(DateTime,"12:00:00"), format="%m/%d/%Y %H:%M:%S", tz="EST"),
         DateTime = ifelse(is.na(DateTime_temp), DateTime_temp2, DateTime_temp),
...
bv_wl_21 <- gsheet::gsheet2tbl("https://docs.google.com/spreadsheets/d/1ZeWmGQHiOYXUDrpeqgkGeA-KOBTq3-SDSPCZfeLz3NQ/edit#gid=0") |> 
  mutate(WaterLevel_ft = as.numeric(WaterLevel_ft),
         DateTime_temp = as.POSIXct(DateTime, format="%m/%d/%y %H:%M", tz="EST"),
         DateTime_temp2 = as.POSIXct(paste0(DateTime,"12:00:00"), format="%m/%d/%Y %H:%M:%S", tz="EST"),
         DateTime = ifelse(is.na(DateTime_temp), DateTime_temp2, DateTime_temp),
...
bv_wl_22 <- gsheet::gsheet2tbl("https://docs.google.com/spreadsheets/d/1y6pI3BsWh_uM1WBQc2WCAkG1eFbScWTvvBv--jQ2WIY/edit#gid=0") |> 
    mutate(WaterLevel_ft = as.numeric(WaterLevel_ft),
         DateTime_temp = as.POSIXct(DateTime, format="%m/%d/%y %H:%M", tz="EST"),
         DateTime_temp2 = as.POSIXct(paste0(DateTime,"12:00:00"), format="%m/%d/%Y %H:%M:%S", tz="EST"),
         DateTime = ifelse(is.na(DateTime_temp), DateTime_temp2, DateTime_temp),
...
#merge bvr wl files
bvr_wl_obs <- dplyr::bind_rows(bv_wl_19,bv_wl_20,bv_wl_21,bv_wl_22) |> 
...
# Name of where to save the file

#output <- "/Users/heatherwander/Documents/VirginiaTech/research/BVR_GLM/bvr_glm/inputs/water_level/BVR_WaterLevel_2015_2022_interp.csv"
output <- "BVR_WaterLevel_2015_2022_interp.csv"

write.csv(wl_final,output , row.names = F)
...
#out_file <- "/Users/heatherwander/Documents/VirginiaTech/research/BVR_GLM/bvr_glm/inputs/BVR_Daily_WaterLevel_Vol_2015_2022_interp.csv"

out_file <- "BVR_Daily_WaterLevel_Vol_2015_2022_interp.csv"

#export file
write.csv(bvr_wl_final, out_file, row.names = F)
