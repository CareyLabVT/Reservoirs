...
# Read in the Google Drive file
 ...
 bvr <- bvr_wl%>%
   ...
   dplyr::mutate(Time = format(as.POSIXct(DateTime), format = "%H:%M:%S"),
   ...
   dplyr::mutate(
     ...
     Flag_WaterLevel_m = Flag_Obs_waterlevel_from_full_pond_ft)%>%
   dplyr::select(Reservoir, Site, Source, DateTime, Obs_waterlevel_from_full_pond_ft, 
                 WaterLevel_m, Flag_DateTime, Flag_Obs_waterlevel_from_full_pond_ft, Flag_WaterLevel_m)
 
 # convert the datetime to a character and save
 
 bvr$DateTime <- as.character(format(bvr$DateTime))
 
 write_csv(bvr, "./Data/DataNotYetUploadedToEDI/Raw_waterlevel/BVR/BVR_WaterLevel_2009_2024.csv")
