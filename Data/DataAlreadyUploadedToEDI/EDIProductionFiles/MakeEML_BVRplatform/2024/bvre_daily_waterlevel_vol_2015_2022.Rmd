...
inUrl1  <- "https://pasta.lternet.edu/package/data/eml/edi/725/4/9adadd2a7c2319e54227ab31a161ea12" 
infile1 <- tempfile()
...
sensor_depth = sensor%>%
  ...
  filter(year(DateTime)>=2022,
         ...
         LvlDepth_m_13>7.5)%>%
  arrange(DateTime)%>%
  filter(!lag(LvlDepth_m_13)-LvlDepth_m_13>0.2)%>%
  mutate(Date = as.Date(DateTime))%>%
  group_by(Date)%>%
  summarize(WaterLevel = mean(LvlDepth_m_13))%>%
  mutate(Year = year(Date),
         Site = 50)


sensor_depth = sensor%>%
  mutate(WaterLevel=LvlDepth_m_13)%>%#the sensor is 0.5m off the bottom
  #mutate(DateTime = as.POSIXct(TIMESTAMP),
         #WaterLevel = Lvl_psi*0.70455)%>%
...
plot(depth$WaterLevel_m, depth$LvlDepth_m_13)

lm_depth<- lm(WaterLevel_m~LvlDepth_m_13, data=depth)

summary(lm_depth)

depth$check=-0.247+ (1.043*depth$LvlDepth_m_13)
...
vt_obs<- bvr_wl_obs%>%
  select(DateTime,WaterLevel_m, Source)%>%
  filter(DateTime>ymd_hms("2014-04-01 00:00:00") & DateTime<ymd_hms("2024-01-01 00:00:00"))%>%
  mutate(Date=as.Date(DateTime))%>%
...
ts <- seq(as.Date('2014-04-01'), as.Date('2020-06-17'), by = "day")
ts2 <- data.frame(Date = ts)

# Join water level data with list of dates+time
wl_pre_sensors <- left_join(ts2, wl_final, by = 'Date')%>%
  mutate(WaterLevel_m = ifelse(Date=="2014-04-01", 13.5, WaterLevel_m)) %>% #make first date be 13.5 for interpolation
...
write.csv(wl_final, "/Users/heatherwander/Documents/VirginiaTech/research/BVR_GLM/bvr_glm/inputs/water_level/BVR_WaterLevel_2014_2023_interp.csv", row.names = F)
...
#export file
write.csv(bvr_wl_final, "/Users/heatherwander/Documents/VirginiaTech/research/BVR_GLM/bvr_glm/inputs/BVR_Daily_WaterLevel_Vol_2015_2023_interp.csv", row.names = FALSE)
