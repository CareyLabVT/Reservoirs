...
If you are REVIEWING this data package, add the pasta URL from EDI in the "EDI HERE" chunk and change the role to "reviewer". Then knit the file which will produce a html to look at all the plots. This is the easiest way to make the plots because it will ignore chunks that the datat product lead needs to run.  
...
1. Make sure all the date ranges are up to date especially current_time_start and current_time_end and the file names are up to date in the EDIT HERE chunk

2. Update the links in the QAQC function to include all raw files both streaming and maual downloads. 
...
CATPRES_COL_NAMES =  c("DateTime","Record", "CR3000Battery_V", "CR3000Panel_Temp_C", 
  ...
  raw <- read_csv(c("https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/ccre-dam-data/ccre-met.csv"), skip = 1, col_names = CATPRES_COL_NAMES,
...
# Make a wind plot of the raw wind direction to compare with corrected values. 
 ...
 # Visualize wind directions for all obs by Year     
      ...
      # Make plot
      pollutionRose(chicago_wind, pollutant="ws", type = "year", main= "Raw Wind Direction After Met Station fell over and is not corrected")
