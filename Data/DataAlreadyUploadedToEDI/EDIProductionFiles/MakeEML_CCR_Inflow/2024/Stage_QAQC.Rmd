...
maint_log$TIMESTAMP_start <- force_tz(as.POSIXct(maint_log$TIMESTAMP_start), tzone = "EST")
maint_log$TIMESTAMP_end <- force_tz(as.POSIXct(maint_log$TIMESTAMP_end), tzone = "EST")
...
waterlevel_correct <- left_join(hobo_qaqc, ccr_met, by = "DateTime_EST") |> 
  ...
  select(DateTime_EST, waterlevel_cm, Flag)#convert pressure to water depth
...
rain_comp <- left_join(waterlevel_correct, ccr_met, by = "DateTime_EST")
...
full_join(waterlevel_correct, stage_checks, by = "DateTime_EST") |> 
...
This section will eventually correct PT sensor data to manual measurements. For the first two deployments, the manual measurements were in reference between a cobble bottom and riffle surface, so I don't think it is worth correcting to. 

PT 1 has fairly equal spread of higher and lower values, might show that the PT was slightly under the cobbles bc it was dug in with the stilling well, but imo not enough of a consistent offset to correct for.

PT 2 seems to have a bias towards deeper manual measurements than PT readings. All being fairly significant differences. Again, I would not correct for each manual measurement, but bc of the fairly regular looking "negative" stages, I think we can pick a consistent offset to apply. This will also help our agreement with the manual measurements.
...
Both measured values between PT downloads show large differences (-13.6 cm and -15.17 cm off), apply a constant offset (#+ 14.385 cm) to the PT data 
...
##Correct to Manual Measurements
This section can eventually be written to correct PT data to the manually measured water level. Not for the current data. 

##Visualize final corrected data to precip, nearby USGS gage, etc
```{r}
a <- waterlevel_correct |> 
  ggplot(aes(x = DateTime_EST, y = waterlevel_cm))+
  geom_point() +
...
ggplot(data = rain_comp, aes(x = DateTime_EST)) +
  geom_point(aes(y = waterlevel_cm), size = 0.5) +
  geom_col(aes(y = Rain_Total_mm*10), color = "blue") + #x 10 rain for better viewing
  scale_x_datetime() +
  coord_cartesian(
    xlim = c(ymd_hms("2025-04-16 00:00:00"), ymd_hms("2025-06-11 23:59:59")))
