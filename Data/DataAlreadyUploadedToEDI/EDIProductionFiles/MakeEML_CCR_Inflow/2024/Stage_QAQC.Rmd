...
pacman::p_load(dplyr, tidyverse, httr2, jsonlite, plotly, googlesheets4, ggpmisc, rLakeAnalyzer)
...
# Can download and save locally if the link from EDI isn't working. Just paste the pasta link into your internet browser
# ccr_met_edi <- read_csv("ccre-met_2021_2025.csv")
...
# Check the corrected pressure and take out the times when it is out of the water
...
# for loop to create flag columns
  for(j in colnames(hobo |> select(HOBO_Abs_Pres_kPa:Temp_C))) { 
    ...
    hobo[c(which(is.na(hobo[,j]))),paste0("Flag_",j)] <-2 #puts in flag 7 if value not collected
...
stage_final <- hobo|>
  ...
  mutate(Flag_waterlevel_cm = 0,
         ...
         waterlevel_cm = ifelse(waterlevel_cm < 0, 0, waterlevel_cm),
...
for(j in colnames(stage_final |> select(HOBO_Abs_Pres_kPa:Temp_C))) { 
    
    stage_final[c(which(is.na(stage_final[,j] & is.na(stage_final[,paste0("Flag_",j)])))),paste0("Flag_",j)] <- 2 #puts in flag 7 if value not collected
  }
...
ggplot()+
  geom_point(data = stage_final, aes(x = DateTime, y = waterlevel_cm))+
  geom_point(data = stage_checks, aes(x = DateTime, y = manual_depth_cm), color = "red")+
...
rain_comp <- left_join(stage_final, ccr_met_edi, by = "DateTime")
