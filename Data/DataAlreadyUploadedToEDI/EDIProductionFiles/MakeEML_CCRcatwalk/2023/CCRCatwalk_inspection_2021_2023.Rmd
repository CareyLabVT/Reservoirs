...
# Script to add depths and take out observations that are out of the water
...
# Since we didn't publish 2022, using the staged data. 
...
if (is.na(file.size(infile1))) download.file(inUrl1,infile1,method="auto")
 ...
 # need to rename some columns here

```

```{r Read in current L1 file, include=FALSE}

# L1 <- read_csv("CCRCatwalk_L1.csv")
 L1 <- read_csv("https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/ccre-dam-data-qaqc/ccre-waterquality_L1.csv")
...
The minimum water level for BVR was `r min(ccrwater$LvlDepth_m_13, na.rm=T)` and the maximum water depth was `r max(ccrwater$LvlDepth_m_13, na.rm=T)`.
...
# make the plots
outputs <- lapply(dx, all_plot, y_lab = "RFU or QSU", y_lab2 = "RFU or QSU",Water = T, Raw_file = T, Use_plotly=F)
...
# make the plots
outputs <- lapply(dx, all_plot, y_lab = "uScm or mg/L", y_lab2 = "uScm or mg/L", Water= T, Raw_file=T, Use_plotly=F)
