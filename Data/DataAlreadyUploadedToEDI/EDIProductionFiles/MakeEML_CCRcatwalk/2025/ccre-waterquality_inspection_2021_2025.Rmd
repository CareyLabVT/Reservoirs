...
2. Change the years in the EDIT HERE chunk.

3. Update the data frame in "Get raw files" to include the most recent file on GitHub.

4. Knit the file. This will take a long time. 

5. Look over the plots and see if there are any issues that need to be added to the maintenance log. I will usually read in the file you just made and make smaller plots with the variable and month in question. Once I have the dates, add them to the maintenance log. 

6. Re-run the inspection script until you have found all of the issues. 

7. Run the "Download and save Maintenance Log" chunk to have the most up to date files in the EDI folder.

8. Make sure large maintenance issues are also documented in the methods.txt file.

```{r Set Up, include=FALSE}
if (!require("pacman")) install.packages("pacman")
pacman::p_load(RCurl,devtools, tidyverse,lubridate, plotly, magrittr, scattermore, knitr, htmltools, pander, rquery)
...
# Update the name of the data package. This should stay the same year to year. If you need to change names of the downloaded files go to chunk 33. 

# data package name

data_package <- "ccre-waterquality"
...
current_df <-qaqc_ccr(
    data_file = c('https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/refs/heads/ccre-dam-data/ccre-waterquality.csv', 'https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/ccre-dam-data/ccre-waterquality2.csv'),
    EXO2_manual_file = 'https://raw.githubusercontent.com/CareyLabVT/ManualDownloadsSCCData/refs/heads/master/CCR_manual_downloads/CCRWaterquality_manual_2021_2025.csv.csv',
...
# Add the turbidity columns to the header list
  ...
  raw <- read_csv("https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/refs/heads/ccre-dam-data/ccre-waterquality.csv", skip = 1, col_names = CATPRES_COL_NAMES,
                      col_types = cols(.default = col_double(), DateTime = col_datetime()))|>
    filter(DateTime < as.Date("2025-04-01"))|>
    mutate(EXOTurbidity_FNU_1 = NA)
  
  # Add in the raw data from before I added a new column to the data frame. Can remove next year, 2027
  
  data_file2 <- read_csv("https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/refs/heads/ccre-dam-data/ccre-waterquality2.csv", skip = 1, col_names = CATPRES_COL_NAMES2,
                      col_types = cols(.default = col_double(), DateTime = col_datetime()))|>
    select(DateTime, starts_with("Th"), starts_with("Lvl"))
  
  
  # Take out extremely high values, EXO_Date and Time, and create depth column
   raw <- bind_rows(raw, data_file2)|>
...
# Check for any gaps longer than 30 minutes in the record for the current year
 ...
 ccre2 <- current_df%>%
  ...
  for(i in 2:length(ccre2$EXOTemp_C_1)){ #this identifies if there are any data gaps in the long-term record, and where they are by record number
    if(abs(difftime(ccre2$DateTime[i], ccre2$DateTime[i-1], units="mins")>30)){
      print(c(ccre2$DateTime[i-1], ccre2$DateTime[i]))
    }
  }
...
# Check for any gaps longer than 30 minutes in the record for the current year
 ...
 ccre2 <- current_df%>%
  ...
  for(i in 2:length(ccre2$EXOTemp_C_9)){ #this identifies if there are any data gaps in the long-term record, and where they are by record number
    if(abs(difftime(ccre2$DateTime[i], ccre2$DateTime[i-1], units="mins")>30)){
      print(c(ccre2$DateTime[i-1], ccre2$DateTime[i]))
    }
  }
...
Let's look at the flag Frequency for each variable. As a reminder here are the flag codes
 ...
 4: value greater than 2 or 4 standard deviations from closets points and set to NA
 
 5: questionable value but left in the dataset
 
 6: Values adjusted, see ccre-maintenancelog for specific adjustments
...
dx <- colnames(current%>%select(grep("EXOTurbidity_FNU_1",colnames(current))))

# make the plots
outputs <- lapply(dx, all_plot, data= current_df, raw_data = raw, reservoir = "CCR", y_lab = "FNU", y_lab2 = "uScm or mg/L", Use_plotly=F)
...
```{r Print plotly turb, echo=FALSE, messages=FALSE, warning=FALSE}
...
write_csv(current_df, paste0(data_package,"_2021_",year,".csv"))
...
# Maintenance Log
download.file("https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/ccre-dam-data-qaqc/CCRW_MaintenanceLog.csv", paste0(data_package,"_maintenancelog_2021_",year,".csv"))

# qaqc function
download.file("https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/ccre-dam-data-qaqc/R/edi_qaqc_function.R", paste0(data_package,"_qaqc_2021_",year,".R"))
...
download.file("https://raw.githubusercontent.com/FLARE-forecast/CCRE-data/ccre-dam-data-qaqc/CCR_Depth_offsets.csv",
              paste0(data_package,"_depth_offsets_2021_",year,".csv"))

# read the file in and then save to take out the blank rows GitHub puts in 

depth <- read_csv(paste0(data_package,"_depth_offsets_2021_",year,".csv"))

write.csv(depth, depth_offset, row.names = F)
...
download.file("https://raw.githubusercontent.com/CareyLabVT/ManualDownloadsSCCData/refs/heads/master/CCR_manual_downloads/CCR_Hobos/Site50_water_temp/CCR_hobos_20_21.csv", "ccre-waterquality-hobos_2020_2021.csv")

# Download the HOBO compilation script from GitHub
download.file("https://raw.githubusercontent.com/CareyLabVT/ManualDownloadsSCCData/refs/heads/master/CCR_manual_downloads/CCR_Hobos/Site50_water_temp/CCR_hobo_compilation.R", "ccre-waterquality-hobo_compilation_2020_2021.R")
