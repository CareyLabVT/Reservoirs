...
myfiles = list.files(path=paste0(mydir,"2022_processed_files/"), pattern="", full.names=TRUE)
...
ec_all<-ec_all%>%filter(date<as.Date("2023-01-01"))
...
bigcsv$datetime <- as.POSIXct(paste(bigcsv$date , paste(bigcsv$time), sep=" "))
