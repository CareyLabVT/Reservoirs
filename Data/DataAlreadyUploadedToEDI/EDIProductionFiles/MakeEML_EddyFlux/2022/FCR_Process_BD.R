...
mydir = "./Data/DataAlreadyUploadedToEDI/EDIProductionFiles/MakeEML_EddyFlux/"
#wd <- getwd()
#setwd(wd)
...
ec[ec ==-9999] <- NA # Remove -9999 and replace with NAs

# Format time. Make sure it is in EST
ec$datetime <- as.POSIXct(paste(ec$date, ec$time), format="%Y-%m-%d %H:%M", tz="EST")
ec$datetime <- as_datetime(ec$datetime, tz="EST")
...
# Take out duplicated datetimes
ec2 <- ec2[!duplicated(ec2[c('datetime')]),] 

# Make sure time stamps are okay! This is a plot
...
# 69% data for CO2; 74% data for CH4 for 2020-2022
...
############# Reading in data from the Met Station #################
...
met_all <- #read_csv("Data/DataAlreadyUploadedToEDI/EDIProductionFiles/MakeEML_FCRMetData/2022FCR_Met_final_2015_2022.csv")%>%
  read_csv("./Data/Met_final_2015_2022.csv", header=T) %>%
...
met_30_2 <- met_30_2 %>% 
  select(-Breaks) %>% 
  mutate(DateTime_Adj = round_date(DateTime,"30 minutes"))%>%
  #mutate(DateTime_Adj = DateTime + 30)
  select(-DateTime) %>% 
  dplyr::rename(datetime = DateTime_Adj, AirTC_Avg = AirTemp_C_Average, RH = RH_percent, Pressure = BP_Average_kPa, 
...
# Compare wind speeds from EC and Met. This wasn't working for me
...
linearModWind <- lm(ec2$wind_speed_ms ~ met2$WS_ms_Avg)
summary(linearModWind)
cor(ec2[[49]],met2[[10]],use = "complete.obs",method=c("pearson"))

# UPDATE EVERY YEAR WITH NEW RELATIONSHIP. 
# Write it down HERE:
# For data period: EC = Met*0.505639+0.207384

# This makes the updated intercept and names it int
# Pull out intercept
wind_int<-as.numeric(linearModWind$coefficients[1])  

# Pull out slope
wind_slo<-as.numeric(linearModWind$coefficients[2])

# Check conversion - UPDATE EVERY YEAR WITH NEW RELATIONSHIP. This is automated and done for you
plot(ec2$wind_speed_ms)
points(met2$WS_ms_Avg*wind_slo+wind_int, col = 'red')

######## Use converted wind speed from the Met data to fill in time points with missing ############
# data (EC)
# UPDATE EVERY YEAR WITH NEW RELATIONSHIP. This is automated from line 162 and 165 
ec2$wind_speed_ms <- ifelse(is.na(ec2$wind_speed_ms),
                         met2$WS_ms_Avg*wind_slo+wind_int, ec2$wind_speed_ms)
...
ec_filt %>% select(datetime, co2_flux_umolm2s, ch4_flux_umolm2s) %>% 
  summarise(co2_available = 100- sum(is.na(co2_flux_umolm2s))/n()*100,
            ch4_available = 100-sum(is.na(ch4_flux_umolm2s))/n()*100)
# Now have 48% CO2 data and 53% CH4 data
...
# Now have 23107 CO2 data points and 25273 CH4 data points
...
ec_filt %>% select(datetime, co2_flux_umolm2s, ch4_flux_umolm2s) %>% 
  summarise(co2_available = 100- sum(is.na(co2_flux_umolm2s))/n()*100,
            ch4_available = 100-sum(is.na(ch4_flux_umolm2s))/n()*100)
# Now have 32% CO2 data and 30% CH4 data
...
# Number of timepoints that have data 
# Now have 15137 Co2 observations and 14500 CH4 observations
################################################################

# Remove CH4 when it rains
# needed to make a new data frame and join because ec_filt and met2 don't have the same 
...
# Number of timepoints 15137 CO2 , 14293 CH4

# Data availability by month
eddy_fcr_season<-eddy_fcr %>% group_by(year(datetime), month(datetime)) %>% select(datetime, co2_flux_umolm2s, ch4_flux_umolm2s) %>% 
  summarise(co2_available = 100-sum(is.na(co2_flux_umolm2s))/n()*100,
            ch4_available = 100-sum(is.na(ch4_flux_umolm2s))/n()*100)
...
# Use linear model to estimate EC temp from Met temp use df test
linearModTemp <- lm(eddy_fcr$air_temp_celsius ~ met2$AirTC_Avg)
summary(linearModTemp)
cor(eddy_fcr[[88]],met2[[3]],use = "complete.obs",method=c("pearson"))
# EC = Met*0.952148-1.093324
# UPDATE RELATIONSHIP EACH YEAR!!

# This makes the updated intercept and names it temp_int
# Pull out intercept to use below
temp_int<-as.numeric(linearModTemp$coefficients[1])  

# Pull out slope to use below
temp_slo<-as.numeric(linearModTemp$coefficients[2])

eddy_fcr$air_temp_celsius <- ifelse(is.na(eddy_fcr$air_temp_celsius),
                                    met2$AirTC_Avg*temp_slo+temp_int, eddy_fcr$air_temp_celsius)
...
linearModSon <- lm(eddy_fcr$sonic_temp_celsius ~ met2$AirTC_Avg)
summary(linearModSon)
cor(eddy_fcr[[89]],met2[[3]],use = "complete.obs",method=c("pearson"))
# EC = Met*1.024114-0.612040
# UPDATE RELATIONSHIP EACH YEAR!!

# This makes the updated intercept and names it temp_int
# Pull out intercept to use below
son_int<-as.numeric(linearModSon$coefficients[1])  

# Pull out slope to use below
son_slo<-as.numeric(linearModSon$coefficients[2])

eddy_fcr$sonic_temp_celsius <- ifelse(is.na(eddy_fcr$sonic_temp_celsius),
                                      eddy_fcr$air_temp_celsius*son_slo+son_int, eddy_fcr$sonic_temp_celsius)
...
linearModRH <- lm(eddy_fcr$RH ~ met2$RH)
summary(linearModRH)
cor(eddy_fcr[[46]],met2[[4]],use = "complete.obs",method=c("pearson"))
# EC= Met*0.792932 + 14.568929
# EC = Met*0.845661 + 7.712944 - old
# UPDATE EACH YEAR!!

# This makes the updated intercept and names it temp_int
# Pull out intercept to use below
rh_int<-as.numeric(linearModRH$coefficients[1])  

# Pull out slope to use below
rh_slo<-as.numeric(linearModRH$coefficients[2])

eddy_fcr$RH <- ifelse(is.na(eddy_fcr$RH),
                      met2$RH*rh_slo+rh_int, eddy_fcr$RH)

# Add Met data to gapfill fluxes
#eddy_fcr<-met2%>%
#  select(datetime, SW_in, SW_out, PAR_Tot_Tot, Pressure, LW_in, LW_out, albedo)%>%
#  left_join(eddy_fcr, . , by="datetime")%>%
#  dplyr::rename(par_tot=PAR_Tot_Tot, air_pressure=Pressure)
...
# 11334 CO2 obs available, 11063 CH4 obs available
...
write_csv(fcr_gf, "./Data/20230121_EC_processed.csv")
