...
# 74.62% data for CO2; 76.4% data for CH4
...
### Added from EDI 2026:
We want to visualize the percentage availability of co2 and ch4 data each month through all the years.

```{r}
#compute monthly availability - slightly modified from above
availability_monthly <- ec2 %>%
  mutate(
    year  = year(datetime),
  ...
  group_by(year, month) %>%
  summarise(
    co2_available = 100 * mean(!is.na(co2_flux_umolm2s)),
    ch4_available = 100 * mean(!is.na(ch4_flux_umolm2s)),
    .groups = "drop"
  )

#convert to long format for plotting
availability_long <- availability_monthly %>%
  pivot_longer(
    cols = c(co2_available, ch4_available),
    names_to = "gas",
    values_to = "availability"
  )

#plot
ggplot(availability_long,
       aes(x = month, y = year, fill = availability)) +
  geom_tile(color = "white") +
  facet_wrap(~ gas, ncol = 1) +
  scale_fill_viridis_c(
    name = "Data availability (%)",
    limits = c(0, 100)
  ) +
  labs(
    title = "Monthly availability of CO2 and CH4 flux data - Only EddyPro Processed",
    x = "Month",
    y = "Year"
  ) +
...
```{r Inital Flux plots, echo=FALSE, warning=FALSE}
...
# Read in Met file from EDI. Data from 2015-2025. Update the date range when you update the link. 
...
```{r Compare wind speeds from EC and Met, warning= FALSE}
#There could be a data row mismatch here between ec2 and met2 (e.g. due to daylight savings duplication) which can lead to error. Uncomment and run the following code if such thing happens.
#check for duplicates (might need to change the file names)
#df_long %>% count(datetime) %>% filter(n > 1)
#df_short %>% count(datetime) %>% filter(n > 1)

#This removes the second duplicate and keeps the first one (might need to change the file names)
#ec2 <- ec2 %>%
...
# For data period: EC = Met*0.546179+0.140389
...
ec_filt %>% select(datetime, co2_flux_umolm2s, ch4_flux_umolm2s) %>% 
  summarise(co2_available = 100- sum(is.na(co2_flux_umolm2s))/n()*100,
            ch4_available = 100-sum(is.na(ch4_flux_umolm2s))/n()*100)
# Now have 51.34% CO2 data and 53.85% CH4 data
...
ec_filt %>% select(datetime, co2_flux_umolm2s, ch4_flux_umolm2s) %>% 
  summarise(co2_available = 100- sum(is.na(co2_flux_umolm2s))/n()*100,
            ch4_available = 100-sum(is.na(ch4_flux_umolm2s))/n()*100)
# Now have 33.39% CO2 data and 30.96% CH4 data
...
# Remove flux data when sonic temperature values were faulty (July 18 - July 21)
ec_filt <- ec_filt %>%
  filter(
    !(datetime >= as.POSIXct("2025-07-18 17:30") &
      datetime <= as.POSIXct("2025-07-21 09:30"))
  )
