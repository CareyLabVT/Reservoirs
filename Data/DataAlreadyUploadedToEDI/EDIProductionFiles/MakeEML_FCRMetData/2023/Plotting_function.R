...
all_plot<-function(Var, 
  ...
  if(Res=="FCR" & Var %in% c("ThermistorTemp_C_5","ThermistorTemp_C_9")){
    ...
    if(grepl("Battery_V|power_V", Var)|Use_plotly==FALSE){
      ...
      cur <- ggplot()+
        {if(switch_raw)geom_scattermore(data= raw_current, aes(x=DateTime, y=.data[[Var]], color=type), pointsize = 3)}+
    ...
    cur <- {ggplot()+
        {if(switch_raw) geom_point(data= raw_current, aes(x=DateTime, y=.data[[Var]], color=type))}+
