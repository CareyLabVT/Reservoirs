...
edi_link <- "https://pasta-s.lternet.edu/package/data/eml/edi/997/18/98f19e7acae8bea7d127c463b1bb5fbc" 
...
ghgplotting <- function(data_df,
  ...
  # Let's first split the data frame if we need to 
  
  if(!is.null(starttime)){
    data_df <- data_df|>
      filter(DateTime >= starttime & DateTime <= endtime)
  }
...
# make the plot  
 b <-  ggplot(a, aes(x = DateTime, y = .data[[var]], colour = as.factor(Depth_m), shape = as.factor(.data[[paste0("Flag_",var)]])))+
  geom_point() +
   {if(length(site)>0)facet_grid(Site ~ ., scales="free_y")}+
  labs(title = paste0(reservoir," "," ",var))+
   theme_bw()
...
# make the plots
outputs <- lapply(c("CH4_umolL", "CO2_umolL"), ghgplotting, data_df = current_df, reservoir = "FCR", site = 50, starttime = NULL, endtime = NULL, plotly = F)
...
# make the plots
outputs <- lapply(c("CH4_umolL", "CO2_umolL"), ghgplotting, data_df = current_df, reservoir = "FCR", site = fgh, starttime = NULL, endtime = NULL, plotly = F)
...
```{r All Current FCR Site 50 Plots All data, echo=FALSE}

# make the plots
outputs <- lapply(c("CH4_umolL", "CO2_umolL"), ghgplotting, data_df = current_df, reservoir = "FCR", site = 50, starttime = current_start, endtime = current_end, plotly = F)

outputs

```

```{r All Current other FCR Sites, echo=FALSE}

# make a list of Sites in FCR except for Site 50 

fcr_sites <- current_df|> 
  filter(Reservoir == "FCR" & Site != 50 &
           DateTime >= current_start & DateTime <= current_end)|>
  select(Site)|>
  unique()
...
# make the plots
outputs <- lapply(c("CH4_umolL", "CO2_umolL"), 
                  ghgplotting, 
                  data_df = current_df, 
                  reservoir = "FCR", 
                  site = fgh, 
