...
# Plot CH4 at Site 50 in CCR
current_df |>  
  filter(Depth_m<100,  Site == 50|Site==51,
         Reservoir == "CCR") |> 
    mutate(Depth_m = as_factor(Depth_m)) |> 
  ggplot(aes(x = DateTime, y = CH4_umolL, colour = Depth_m))+
  geom_point() +
...
current_df |>  
  ...
  ggplot(aes(x=DateTime, y=CH4_umolL, colour=as.factor(Site)))+
  geom_point() +
  labs(title = 'CCR CH4 other sites')
...
current_df |>  
  ...
  ggplot(aes(x=DateTime, y=CO2_umolL, colour=as.factor(Site)))+
  geom_point() +
  labs(title = 'CCR CO2 other sites')
...
ghgplotting <- function(data,
 ...
 # data= current_df
 #  reservoir = c("BVR", "FCR", "CCR")
 #  site = c(50, 200, 100)
 #  depth = c(0.1, 1.6, 3.8, 5, 6.2, 8, 9, 21)
 #  starttime = as.Date("2025-01-01")
 #  endtime = Sys.Date()
 #  var = "CH4_umolL"
 #  title = "Reservoir CH4 concentration"


  a = data |>  
  filter(Depth_m %in% depth,
         Reservoir %in% reservoir,
         Site %in% site,
         DateTime >= starttime & DateTime <= endtime) |>
  mutate(Site = as.factor(Site)) |> 
  ggplot(aes(x = DateTime, y = .data[[var]], colour = Site))+
  geom_point() +
  labs(title = title)
...
ghgplotting(data=current_df, reservoir= "FCR",site = 50, depth = c(0.1, 1.6, 3.8, 5, 6.2, 8, 9), starttime =
              as.Date("2025-01-01"), endtime = Sys.Date(), var = "CH4_umolL", title = "FCR CH4 concentration"  )
...
e <- current_df |>  
  filter(Depth_m<100,
         Reservoir == "CCR", Site == 50, 
         DateTime >= '2025-01-01') |> 
  mutate(Depth_m = as_factor(Depth_m)) |> 
  ggplot(aes(x = DateTime, y = CH4_umolL, colour = Depth_m))+
  geom_point() +
  labs(title = 'CCR Site 50, CH4')
