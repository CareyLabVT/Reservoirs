...
#
# Added a section to identify gaps in the data NEED TO ADD
...
inflow_pressure <- inflow_pressure %>%
  ...
  mutate(DateTime = parse_date_time(Date, 'dmy HMS')) %>%
  select(-Date) %>%
...
pressure = pressure %>%
  ...
  mutate(DateTime = parse_date_time(Date, '%Y/%m/%d H:M:S')) %>%
...
baro_pressure = baro_pressure %>%
  ...
  arrange(DateTime)%>%
  mutate(DateTime = round_date(DateTime,"15 minutes"))%>% # Round to the nearest 15 min when not an even interval
  distinct()
...
VTdat$DateTime <- ymd_hms(VTdat$DateTime)
...
#### Add Flag columns ### 
All_Inflow$WVWA_Flag_Pressure_psi<-0
All_Inflow$WVWA_Flag_Baro_pressure_psi<-0
All_Inflow$WVWA_Flag_Pressure_psia<-0
All_Inflow$WVWA_Flag_Flow_cms<-0
All_Inflow$WVWA_Flag_Temp_C<-0
All_Inflow$VT_Flag_Pressure_psia<-0
All_Inflow$VT_Flag_Flow_cms<-0
All_Inflow$VT_Flag_Temp_C<-0
...
for(i in 1:nrow(main_log))
{
  ...
  if(flag==1){ # for down correcting
    ...
    All_Inflow <- All_Inflow%>%
      mutate(WVWA_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(WVWA_Pressure_psia) &
                                         WVWA_Pressure_psia<0.184,13, WVWA_Flag_Flow_cms))
    
    # correct flags for those that were down corrected and if the flow is too high and over tops the weir
    All_Inflow <- All_Inflow%>%
      mutate(WVWA_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(WVWA_Pressure_psia) &
                                         WVWA_Pressure_psia>0.611,16, WVWA_Flag_Flow_cms))
...
rating_curve_dates$DateTime <- as.POSIXct(strptime(rating_curve_dates$DateTime, "%m/%d/%Y %H:%M"))
...
rating_curve <- rating_curve%>% select(DateTime,GageHeight_cm, WVWA_Pressure_psia, VT_Pressure_psia)
  ...
  if(info_cols==100){
    ...
    All_Inflow <- All_Inflow%>%
      mutate(WVWA_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(WVWA_Pressure_psia) &
                                         WVWA_Pressure_psia<0.184,NA, main_cols))
    
    # correct flags for flow being too low based on pressure sensor
    All_Inflow <- All_Inflow%>%
      mutate(WVWA_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(WVWA_Pressure_psia) &
                             WVWA_Flag_Flow_cms!=13 & WVWA_Pressure_psia<0.184,3, WVWA_Flag_Flow_cms))
    ...
    All_Inflow <- All_Inflow%>%
      mutate(WVWA_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(WVWA_Pressure_psia) &
                          WVWA_Flag_Flow_cms!=16 & WVWA_Pressure_psia>0.611,6, WVWA_Flag_Flow_cms))
      }else if (main_cols==8){
      # Make flow NA when psi <= 0.184 (distance between pressure sensor and bottom of weir = 0.1298575 m = 0.18337 psi
      All_Inflow <- All_Inflow%>%
        mutate(VT_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(VT_Pressure_psia) &
                                      VT_Pressure_psia<0.184,NA, main_cols))
      
      # correct flags for flow being too low based on pressure sensor
      All_Inflow <- All_Inflow%>%
        mutate(VT_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(VT_Pressure_psia) &
                                            VT_Pressure_psia<0.184,3, VT_Flag_Flow_cms))
      ...
      All_Inflow <- All_Inflow%>%
        mutate(VT_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(VT_Pressure_psia) &
                                            VT_Pressure_psia>0.611,6, VT_Flag_Flow_cms))
    ...
    # Can use this for the rest of the time 
    ...
    sel<- rating_curve[rating_curve$DateTime >= start & rating_curve$DateTime <= end,]
      ...
      if(main_cols==5){ # Let's calculate flow for WVWA
      #get linear relationship between pressure gage and 
     fit <- lm(sel$GageHeight_cm ~ sel$WVWA_Pressure_psia)
     ...
     All_Inflow <- All_Inflow %>% mutate(head = ((slo*WVWA_Pressure_psia)+int)/100) %>% 
     ...
     All_Inflow<-All_Inflow%>%
       mutate(WVWA_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(WVWA_Pressure_psia) &
                                     WVWA_Pressure_psia<=low,NA, WVWA_Flow_cms))%>%
       mutate(WVWA_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(WVWA_Pressure_psia) &
                                     WVWA_Pressure_psia<=low,3, WVWA_Flag_Flow_cms))
     ...
     All_Inflow<-All_Inflow%>%
     mutate(WVWA_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(WVWA_Pressure_psia) &
                                        WVWA_Pressure_psia>high,6, WVWA_Flag_Flow_cms))
     
      }else if (main_cols==8){# Let's do it for VT Sensor
      fit <- lm(sel$GageHeight_cm ~ sel$VT_Pressure_psia)
      plot(sel$GageHeight_cm ~ sel$VT_Pressure_psia)
      ...
      All_Inflow <- All_Inflow %>% mutate(head = ((slo*VT_Pressure_psia)+int)/100) %>% 
      ...
      All_Inflow<-All_Inflow%>%
        mutate(VT_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(VT_Pressure_psia) &
                                      VT_Pressure_psia<=low,NA, VT_Flow_cms))%>%
        mutate(VT_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(VT_Pressure_psia) &
                                           VT_Pressure_psia<=low,3, VT_Flag_Flow_cms))
      ...
      All_Inflow<-All_Inflow%>%
        mutate(VT_Flag_Flow_cms=ifelse(DateTime >= start & DateTime <= end & !is.na(VT_Pressure_psia) &
                                           VT_Pressure_psia>high,6, VT_Flag_Flow_cms))
      }else 
      warning(paste("Could not parse column flagcol in row", i, "of the rating log."))
  }
 }   
#### Lm for gauge height to psi realtionship #####

# V-notch weir post blow-out with FINAL location of the pressure transducer
# Until pressure transducer battery died and was replaced
diff_sep20 <- rating_curve %>% 
  filter(DateTime > ymd_hms('2020-09-02 16:00:00') & DateTime <=ymd_hms('2021-11-08 13:45:00'))

# Pressure transducer battery died and replaced them 
# Sensors removed so they could clean out the weir
diff_nov21 <- rating_curve %>% 
  filter(DateTime > ymd_hms('2021-11-08 13:45:00') & DateTime <= ymd_hms('2022-05-09 14:15:00'))

# Sensors removed so they could clean out the weir
diff_may22 <- rating_curve %>% 
  filter(DateTime > ymd_hms('2022-05-09 14:15:00'))
...
ggplot(data = inflow_pressure, aes(x = DateTime, y = Pressure_psi))+
  geom_point()+
  ylab("Daily avg. inflow pressure (psi)")+
  theme_bw() +
...
ggplot(data = daily_catwalk, aes(x = Date, y = daily_pressure_avg))+
  geom_point()+
  ylab("Daily avg. inflow pressure (psi)")+
  theme_bw() +
  xlim(c(as.Date("2022-01-01"),as.Date("2022-12-31")))
...
downcorrect <- full_join(inflow_pressure, inflow_pressure1,by="DateTime")
...
baro_pressure <- baro_pressure %>% 
  select(Baro_pressure_psi,DateTime)
...
# ABP added DateTimes when the sensors were out of the water
diff <- diff %>%
  ...
  mutate(Baro_pressure_psi = ifelse(DateTime<= "2022-08-16 14:15:00" & DateTime >="2022-07-27 02:45:00", NA, Baro_pressure_psi))%>%
  mutate(Pressure_psia = Pressure_psi - Baro_pressure_psi)

#diff <- diff %>%
#  mutate(Pressure_psia = Pressure_psi - Baro_pressure_psi)
#######ABP come back here and fix back to what AGH had 
a <- diff%>%
  filter(DateTime>"2020-08-01 00:00:00" & DateTime<"2020-08-31 00:00:00")%>%
  ggplot(.,aes(x = DateTime, y = Pressure_psia))+
  geom_point()
#ggplot(., aes(x = as.Date(DateTime), y = Pressure_psia)) +
#geom_point(aes(y=Pressure_psia), color="black") +
#geom_point(aes(y=Pressure_psi), color="red")+
#geom_point(aes(y=Baro_pressure_psi), color="blue")
a
#xlim(c(as.Date("2021-01-01"),as.Date("2022-12-31"))+
#xlim(c(ymd_hms("2021-11-08 00:00:00"),ymd_hms("2021-11-15 00:00:00")))

plot_both <- diff %>%
  mutate(Date = date(DateTime)) 

daily_pressure <- group_by(plot_both, Date) %>% 
  summarize(daily_pressure_avg = mean(Pressure_psi, na.rm=T),
            daily_baro_pressure_avg = mean(Baro_pressure_psi, na.rm=T),
            daily_psia = mean(Pressure_psia, na.rm=T)) %>%
...
# 2021
both_pressures +
  xlim(c(as.Date("2022-01-01"),as.Date("2022-03-31")))

both_pressures +
  xlim(c(as.Date("2022-04-01"),as.Date("2022-06-30")))

both_pressures +
  xlim(c(as.Date("2022-07-01"),as.Date("2022-09-30"))) # End of data record - continue from here!

both_pressures +
  xlim(c(as.Date("2022-10-01"),as.Date("2022-12-31")))
...
diff_plot+
  xlim(as.POSIXct("2021-01-01"),as.POSIXct("2022-12-31"))
...
# Rectangular weir:
diff_pre <- diff[diff$DateTime< as.POSIXct('2019-06-07 02:00:00'),]

# Original installation of V-notch weir
diff_post <- diff %>% 
  filter(DateTime >= as.POSIXct('2019-06-07 02:00:00') & DateTime <= as.POSIXct('2020-08-24 01:00:00'))

# V-notch weir post blow-out but before final pressure transducer location!
diff_aug20 <- diff %>% 
  filter(DateTime > as.POSIXct('2020-08-24 01:00:00') & DateTime <= as.POSIXct('2020-09-02 16:00:00'))

# V-notch weir post blow-out with FINAL location of the pressure transducer
# Until pressure transducer battery died and was replaced
diff_sep20 <- diff %>% 
  filter(DateTime > as.POSIXct('2020-09-02 16:00:00') & DateTime <=as.POSIXct('2021-11-08 13:45:00'))

# Pressure transducer battery died and replaced them 
# Sensors removed so they could clean out the weir
diff_nov21 <- diff %>% 
  filter(DateTime > as.POSIXct('2021-11-08 13:45:00') & DateTime <= as.POSIXct('2022-05-09 14:15:00'))

# Sensors removed so they could clean out the weir
diff_may22 <- diff %>% 
  filter(DateTime > as.POSIXct('2022-05-09 14:15:00'))
...
ggplot(data = Inflow_Final, aes(x = DateTime, y = WVWA_Flow_cms)) +
  geom_line() +
  xlim(c(as.POSIXct('2021-01-01 00:00:00'), as.POSIXct('2022-12-31 00:00:00')))
######################
...
### Let's see what is happening to when VT sensor is whacy in oct and nov 2022

check <-VTdat%>%
  merge(., diff, x, by.x= "DateTime", by.y="DateTime", all=T)%>%
  filter(DateTime>"2021-11-08 00:00")%>%
  mutate(diff_psi=Pressure_psia-VT_Pressure_psia)

negative <- check%>%
  filter(VT_Pressure_psia<0)

b<-check%>%
  filter(DateTime>"2022-11-01 00:00"& DateTime<"2022-12-20 00:00:00")%>%
  ggplot(., aes(x=DateTime))+
  geom_point(aes(y=VT_Pressure_psia),col="red")+
  geom_point(aes(y=Pressure_psia),col="blue")+ 
  geom_point(aes(y=diff_psi), col="black")
...
all_flow_2021 <- all_flow +
  xlim(as.POSIXct('2021-06-01'),as.POSIXct('2022-04-30'))
all_flow_2021

ggplot()+
  geom_line(data=Inflow_Final, mapping=aes(x=DateTime,y=WVWA_Flow_cms,color="WVWA"))+
  #geom_line(data=Inflow_Final,mapping = aes(x=DateTime,y=VT_Flow_cms,color="VT"))+
  xlim(as.POSIXct('2021-12-01'),as.POSIXct('2021-12-31')) +
  ylab("Flow (cms)")+
  theme_bw()+
  mytheme
...
#### ABP update with Maintenance log script so make it easier to update when events happen####
...
# Write to CSV
write.csv(Inflow_Final_10, './Data/DataAlreadyUploadedToEDI/EDIProductionFiles/MakeEML_Inflow/2022_Jun/Inflow_2013_May2022.csv', row.names = FALSE) 
