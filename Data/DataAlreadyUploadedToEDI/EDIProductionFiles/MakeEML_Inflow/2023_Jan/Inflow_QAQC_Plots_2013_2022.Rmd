...
Master QAQC script in prep for publishing FCR Met station data to EDI this script using the qaqc_weir function which automatically QAQCs the data. This script produces figures to look over the data and visually inspect for major outlines. 
...
13: value down corrected due to low flows on the rectangular weir;
...
ggplot(data = weirdata)+
  ...
  theme_bw()+
  ggtitle("All VT senor pressure (psi)") 
...
ggplot(data = weirdata, aes(x = DateTime, y = VT_Pressure_psia))+
  ...
  ylab("Daily avg. VT pressure (psi)")+
  theme_bw() +
  ggtitle("Current VT senor pressure (psi)") +
  xlim(c(ymd_hms(current_time_start),ymd_hms(current_time_end)))
