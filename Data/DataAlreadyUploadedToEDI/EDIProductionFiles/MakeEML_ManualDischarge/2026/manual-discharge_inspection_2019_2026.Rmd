---
title: "manual-discharge_inspection_2019_2026"
...
pacman::p_load(tidyverse, lubridate, gsheet, dplyr, readr, htmltools, ggplot, plotly)
...
## Date trimming variables
start_date <- min(staged_edi$DateTime)
end_date <- max(staged_edi$DateTime)
start_trim_date <- as.POSIXct("2025-10-01")
end_trim_date <- as.POSIXct("2026-12-31")
...
# MAKE SURE TO UPDATE THE PASTA FROM THE VERSION YOU WANT. 
                                                                ...
                                                                ### CHANGE THIS LINK BELOW 
                                                                             ##      
 #staged_edi <-read_csv("https://pasta-s.lternet.edu/package/data/eml/edi/1017/22/0e7fe16623a1ad2a67774c23ce8a29d8")
...
```
#Create List of Active Sites for Current Year
This prints the list of all sites that have data for the current time frame

```{r ID active sites}
# Summarize sites and count distinct days
all_sampled_sites <- staged_edi %>%
  filter(
    DateTime >= start_date,
    DateTime <= end_date,
    !is.na(Flow_cms)  # only rows with actual measurements
  ) %>%
  mutate(Date = as_date(DateTime)) %>%  # extract just the date
  group_by(Reservoir, Site) %>%         # group by location
  summarise(
    n_days_sampled = n_distinct(Date),  # count distinct days
    .groups = "drop"
  ) %>%
  arrange(Reservoir, Site)

all_sampled_sites

current_sampled_sites <- staged_edi %>%
  filter(
    DateTime >= start_trim_date,
    DateTime <= end_trim_date,
    !is.na(Flow_cms)  # only rows with actual measurements
  ) %>%
  mutate(Date = as_date(DateTime)) %>%  # extract just the date
  group_by(Reservoir, Site) %>%         # group by location
...
current_sampled_sites
```

Plot all data for CCR
```{r all CCR, echo=FALSE}
res <- "CCR"


##NEED LIST OF ALL SITES
active_site_names <- all_sampled_sites %>%
...
for (site in active_site_names) {
  ...
  # skip sites with no data so knit doesn’t die dramatically
  if (nrow(df) == 0) next

  p <- ggplot(df, aes(x = DateTime, y = Flow_cms)) +
    ...
    ggtitle(paste(res, site, "Flow_cms", sep = "_"))

  all_plots[[length(all_plots) + 1]] <- p
}

tagList(lapply(all_plots, ggplotly))
```


Plot active sites for CCR

```{r current CCR, echo=FALSE}
res <- "CCR"

active_site_names <- current_sampled_sites %>%
  filter(Reservoir == res) %>%
  pull(Site)
...
```
```{r}
### Look at data for specified date range

staged_edi %>% 
  ggplot(aes(x = DateTime, y = Flow_cms, color = as.factor(Site), shape = as.factor(Flag_Flow_cms))) + 
  geom_point(aes(color = Site)) +
  ggtitle("All data in specified date range")+
  scale_x_datetime(date_labels = "%b %y")+
  coord_cartesian(xlim = c(start_trim_date, end_trim_date))+
  facet_grid(rows = vars(Reservoir), cols = vars(Site), scale = 'free')+
...
staged_edi %>% 
  ...
  ggplot(aes(x = DateTime, y = Flow_cms, color = as.factor(Site), shape = as.factor(Flag_Flow_cms))) + 
  geom_point(aes(color = Site)) +
  ggtitle("BVR in specified date range")+
  scale_x_datetime(date_labels = "%b %y")+
  coord_cartesian(xlim = c(start_trim_date, end_trim_date))+
  facet_grid(rows = vars(Reservoir), cols = vars(Site), scale = 'free')+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
staged_edi %>% 
  ...
  ggplot(aes(x = DateTime, y = Flow_cms, color = as.factor(Site), shape = as.factor(Flag_Flow_cms))) + 
  geom_point(aes(color = Site)) +
  ggtitle("FCR in specified date range")+
  scale_x_datetime(date_labels = "%b %y")+
  coord_cartesian(xlim = c(start_trim_date, end_trim_date))+
  facet_grid(rows = vars(Reservoir), cols = vars(Site), scale = 'free')+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
staged_edi %>% 
  ...
  ggplot(aes(x = DateTime, y = Flow_cms, color = as.factor(Site), shape = as.factor(Flag_Flow_cms))) + 
  geom_point(aes(color = Site)) +
  ggtitle("FCR 200 in specified date range")+
  scale_x_datetime(date_labels = "%b %y")+
  coord_cartesian(xlim = c(start_trim_date, end_trim_date))+
  facet_grid(rows = vars(Reservoir), cols = vars(Site), scale = 'free')+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
staged_edi %>% 
  ...
  ggplot(aes(x = DateTime, y = Flow_cms, color = as.factor(Site), shape = as.factor(Flag_Flow_cms))) + 
  geom_point(aes(color = Site)) +
  ggtitle("CCR in specified date range")+
  scale_x_datetime(date_labels = "%b %y")+
  coord_cartesian(xlim = c(start_trim_date, end_trim_date))+
  facet_grid(rows = vars(Reservoir), cols = vars(Site), scale = 'free')+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
write.csv(staged_edi, "manual-discharge_2019_2025.csv", row.names = F)
...
# Maintenance Log
download.file("https://raw.githubusercontent.com/CareyLabVT/Reservoirs/refs/heads/master/Data/DataNotYetUploadedToEDI/Raw_Discharge/ManualDischarge_Maintenance_Log.csv", "manual-discharge_maintenancelog_2019_2025.csv")

# qaqc function
download.file("https://raw.githubusercontent.com/CareyLabVT/Reservoirs/refs/heads/master/Scripts/L1_functions/manual_discharge_create.R", "manual-discharge_qaqc_2019_2025.R")
