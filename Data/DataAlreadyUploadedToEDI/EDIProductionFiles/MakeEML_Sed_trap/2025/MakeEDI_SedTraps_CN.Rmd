...
cn7 = read.csv("./2025 sed trap runs/CN_FMI_DXE_raw_spring2025.csv")
#Add new file here
...
cn_processed = cn_raw%>%
  filter(!Name %in% c("aa as unknown", #remove reference sediments
                      ...
                      "zzyzx soil",
                      "aa tht is inaccurateweight_DXE",
                      "zzyzx reference"),
         Method == "very low") %>%
...
high_c_factor <- c('b03jul23_8m_f3', # flagged by FMI as having a high C factor
                      'b03jul23_4m_f3',
                      'f03jul23_8m_f3',
                      'f03jul23_4m_f3',
                      'f19jun23_8m_f3',
...
high_c_factor_IDs <- cn_processed %>% 
  filter(Name %in% high_c_factor) %>% 
  select(Filter1ID, Filter2ID)

high_c_factor_F1 <- high_c_factor_IDs$Filter1ID
high_c_factor_F2 <- high_c_factor_IDs$Filter2ID
# correct these abnormally high C_factor values by substituting the avg C_factor
AvgC_Factor <- cn_processed %>% 
  filter(Name %!in% high_c_factor) %>% 
  summarise(AvgC_Factor = round(mean(C_factor, na.rm = TRUE), digits = 4))

# apply avg C_factor to data
cn_processed2 <- cn_processed %>% 
  mutate(C_pct = if_else(Name %in% high_c_factor,
                         round((C_pct/C_factor)*AvgC_Factor[1,1], digits = 3),
                         C_pct),
         C_factor = if_else(Name %in% high_c_factor, AvgC_Factor[1,1], C_factor))
...
cn_processed2%>%
  ...
  geom_point()+
  facet_grid(rows = vars(layer), cols = vars(Reservoir))

#Final formatting to combine with metals
cn_to_combine = cn_processed2%>%
...
```{r}
frame1 = read.csv("../Filtering logs/FilteringLog_2018_2024.csv")
...
CN_DONE = frame2_complete %>%
  ...
  dplyr::rename(Entered_weight_mg = Weight_mg) %>%
  ...
  dplyr::select(Reservoir, Date_sampled, Depth_m, Site, Duration_days,
                ...
                Flag_TN_g, Flag_TOCFlux_gm2d, Flag_TNFlux_gm2d)%>%
...
```{r}
metals_to_combine = read.csv("../Metals/allMetals_2018_2024.csv")%>%
  mutate(Date=as.Date(Date))

comb_raw = CN_DONE%>%
  dplyr:: rename(Date = Date_sampled)%>%
  full_join(metals_to_combine)

comb_formatted = comb_raw%>%
  dplyr::rename(TOCFlux_gm2d = C_flux_gm2d,
  ...
  mutate(Flag_ICPTLi_mgL = ifelse(is.na(ICPTLi_mgL), 2, Flag_ICPTLi_mgL),
         ...
         Flag_TOCFlux_gm2d = if_else(is.na(TOCFlux_gm2d), 2, Flag_TOCFlux_gm2d),
         Flag_TNFlux_gm2d = if_else(is.na(TNFlux_gm2d), 2, Flag_TNFlux_gm2d),
         Flag_TLiFlux_gm2d = if_else(is.na(TLiFlux_gm2d),2, 1),
         Flag_TNaFlux_gm2d = if_else(is.na(TNaFlux_gm2d),2, 1),
         Flag_TMgFlux_gm2d = if_else(is.na(TMgFlux_gm2d),2, 1),
         Flag_TAlFlux_gm2d = if_else(is.na(TAlFlux_gm2d),2, 1),
         Flag_TKFlux_gm2d = if_else(is.na(TKFlux_gm2d),2, 1),
         Flag_TCaFlux_gm2d = if_else(is.na(TCaFlux_gm2d),2, 1),
         Flag_TFeFlux_gm2d = if_else(is.na(TFeFlux_gm2d),2, 1),
         Flag_TMnFlux_gm2d = if_else(is.na(TMnFlux_gm2d),2, 1),
         Flag_TCuFlux_gm2d = if_else(is.na(TCuFlux_gm2d),2, 1),
         Flag_TSrFlux_gm2d = if_else(is.na(TSrFlux_gm2d),2, 1),
         Flag_TBaFlux_gm2d = if_else(is.na(TBaFlux_gm2d),2, 1))
...
comb_formatted = comb_formatted%>%
  mutate(Flag_Duration_days = 1,
         ...
         Flag_Duration_days = ifelse(Date == "2021-06-28" & Reservoir == "BVR", 
                                     2, 
                                     Flag_Duration_days),
         # flag for adjusted C_factor value
         Flag_TOC_g = if_else(Filter1ID %in% high_c_factor_F1, 3,
                              Flag_TOC_g),
         Flag_TOCFlux_gm2d = if_else(Filter1ID %in% high_c_factor_F1, 3,
                              Flag_TOCFlux_gm2d))

write.csv(comb_formatted,"../sed-traps_2018_2024.csv",row.names=F)
