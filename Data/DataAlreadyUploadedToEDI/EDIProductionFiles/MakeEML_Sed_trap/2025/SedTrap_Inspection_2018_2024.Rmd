---
title: "SedTrap Inspection 2018 - 2024"
...
```{r read in the data file, include=FALSE}
 ...
 current_df <-read_csv('https://raw.githubusercontent.com/CareyLabVT/Reservoirs/refs/heads/master/Data/DataNotYetUploadedToEDI/Sed_trap/CN_Metals_Flux_2018_2024.csv') %>% # this file will change to pasta-s link when in EDI
...
This section checks to make sure each observation has a data flag. It also checks to make sure the frequency of flags match what we expect to see. Note that a flag of 1 means "no issues"
...
fluxes <- sedtraps %>% 
  select(Reservoir, Date, Depth_m, TLi_g:TNFlux_gm2d)%>%
...
cum_mass_plot <- function(
  ...
  flux_current <- get(data_flux)%>%
    ...
    ggplot(.)+
     geom_point(aes(x=DateTime, y=.data[[y_var_flux]]))+
          labs(y = "gm2d") +
          ggtitle(paste0("Current ", element_name, " fluxes")) +
    facet_grid(cols = vars(Reservoir), rows = vars(Layer), scales = 'free_x', space = "free_x") +
    theme_bw()+
...
sed <- read_csv("Inspection_script_function_arguments.csv", show_col_types= F) %>% 
  filter(element_name != 'Silicon') %>% 
  mutate(current_year = 2024) %>% 
  add_row(data_flux = 'sedtraps',
          data_mass = 'fluxes33',
          y_var_flux = 'TOCFlux_gm2d',
          y_var_mass = 'Cumul_TOC_g',
          element_name = 'OC',
          current_year = 2024,
          use_plotly = TRUE) %>% 
    add_row(data_flux = 'sedtraps',
          data_mass = 'fluxes33',
          y_var_flux = 'TNFlux_gm2d',
          y_var_mass = 'Cumul_TN_g',
