...
CN_DONE = frame2_complete%>%
  ...
  dplyr::select(Reservoir,Date_sampled,Depth_m,Site,Duration_days,Filter1ID,Filter2ID,CombinedCollectionVol_L,CombinedFilterVol_L,CombinedSedMass_g,CombinedXSA_m2,C_mass_CN_g,N_mass_CN_g,Flag_CombinedCollectionVol_L,Flag_Filter2ID, Flag_CombinedCollectionVol_L, Flag_CombinedSedMass_g, Flag_TOC_g, Flag_TN_g)%>%
...
comb_formatted = comb_raw%>%
  ...
  dplyr::select(Reservoir, 
                ...
                Flag_ICPTMn_mgL)%>%
  arrange(Date)%>%
  mutate(Flag_ICPTFe_mgL = ifelse(is.na(ICPTFe_mgL)&!is.na(TOC_g),2,Flag_ICPTFe_mgL),
         Flag_ICPTMn_mgL = ifelse(is.na(ICPTMn_mgL)&!is.na(TOC_g),2,Flag_ICPTMn_mgL),
         Flag_TOC_g = ifelse(is.na(TOC_g)&!is.na(ICPTFe_mgL),2,Flag_TOC_g),
         Flag_TN_g = ifelse(is.na(TN_g)&!is.na(ICPTFe_mgL),2,Flag_TN_g))
