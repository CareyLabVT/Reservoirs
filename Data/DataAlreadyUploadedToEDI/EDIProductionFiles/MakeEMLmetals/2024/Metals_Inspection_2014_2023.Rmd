...
```{r QAQC FCR 50 Plots, eval=FALSE, warning=FALSE, include=FALSE, results='asis'}
...
```{r Print plotly F50, eval=FALSE, warning=FALSE, include=FALSE, messages=FALSE}
...
```{r QAQC FCR 100 Plots, eval=FALSE, warning=FALSE, include=FALSE, results='asis'}

dx <- colnames(current_df%>%select(TLi_mgL:SBa_mgL))

# make the plots

outputs <- lapply(dx, all_plot, data=current_df, reservoir = "FCR", res_site=100, y_lab = "mg/L", y_lab2 = "mg/L",Depth=T, Water=F, Use_plotly=F, Heatmap = F)
...
```{r Print plotly F100, eval=FALSE, warning=FALSE, include=FALSE, messages=FALSE}
...
```{r QAQC BVR 50 Plots, echo=FALSE, warning=FALSE, results='asis'}
...
```{r QAQC BVR 40 Plots, echo=FALSE, warning=FALSE, results='asis'}
...
```{r QAQC CCR 50 Plots, echo=FALSE, warning=FALSE, results='asis'}

dx <- colnames(current_df%>%select(TLi_mgL:SBa_mgL))

# make the plots

outputs <- lapply(dx, all_plot, data=current_df, reservoir = "CCR", res_site=50, y_lab = "mg/L", y_lab2 = "mg/L",Depth=T, Water=F, Use_plotly=F, Heatmap = T)
...
```{r Make new CSV with current and historic files, eval=TRUE, include=TRUE}
...
```{r Download and save Maintenance Log, eval=TRUE, include=TRUE}
