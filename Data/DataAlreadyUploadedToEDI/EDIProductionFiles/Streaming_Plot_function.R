...
all_plot<-function(Var, y_lab, y_lab2, Water=T, Use_plotly=F){
  ...
  if(Res=="FCR" & Var %in% c("ThermistorTemp_C_5","ThermistorTemp_C_9")){
    ...
    if(grepl("Battery_V|power_V", Var)|Use_plotly==FALSE){
      ...
      cur <- ggplot()+
        geom_scattermore(data= raw_current, aes(x=DateTime, y=.data[[Var]], color=type), pointsize = 3)+
        geom_scattermore(data= qaqc_current, aes(x=DateTime, y=.data[[Var]], color=type), pointsize = 3)+
    ...
    cur <- {ggplot()+
        geom_point(data= raw_current, aes(x=DateTime, y=.data[[Var]], color=type))+
        geom_point(data= qaqc_current, aes(x=DateTime, y=.data[[Var]], color=type))+
        ggtitle(paste0("Current: ",Var)) +
        labs(y = y_lab2,
             color = "Legend") +
        scale_color_manual(values = colors)+
