...
flo_all%>%
  filter(name == "TotalConc_ugL")%>%
  ggplot(aes(x = DateTime, y = depth, color = max))+
  geom_point()+
  scale_y_reverse()
...
fluora%>%
  filter(Reservoir=="BVR",
         Site==50)%>%
  mutate(DateTime = as.Date(DateTime))%>%
  ggplot(aes(x = DateTime, y = Depth_m, color = TotalConc_ugL))+
  geom_point(size = .5)+
  scale_color_gradientn(colours = blue2green2red(100), na.value="white", name = "Total algal conc")+
  scale_y_reverse()+
  theme_bw()
