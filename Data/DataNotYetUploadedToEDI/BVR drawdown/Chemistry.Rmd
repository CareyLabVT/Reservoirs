...
sum_plot3%>%
  mutate(show_points = ifelse(name == "thermo","n","y"))%>%
  ggplot(aes(x = DateTime, y = value, lty = lty, color = name))+
  #geom_hline(aes(yintercept = y),data=horiz_line, color = "grey20",alpha=0.3,size=1)+
  geom_point(aes(alpha = show_points, color = name))+
  geom_line()+
  scale_alpha_manual(values = c(0,1))+
...
sum_plot4%>%
  ggplot()+
  geom_rect(aes(xmin = opened,xmax = closed,ymin=-Inf,ymax=Inf),fill = "grey20", data = lines_rect, alpha = 0.2)+
