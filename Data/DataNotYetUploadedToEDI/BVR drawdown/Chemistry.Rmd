...
sum_plot4 = chem_full%>%
  ...
  full_join(ghg%>%mutate(DateTime = as.Date(DateTime)))%>%
  select(DateTime, PO4_ugL,DOC_mgL,TotalConc_ugL,pH,NH4_ugL,co2_umolL,DIC_mgL,Bluegreens_ugL)%>%
  pivot_longer(cols = c(PO4_ugL,DOC_mgL,TotalConc_ugL,pH,co2_umolL,NH4_ugL,DIC_mgL,Bluegreens_ugL))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime),
  ...
  mutate(name = factor(name, levels=c("PO4_ugL","Phyto","pH","DOC_mgL","co2_umolL","NH4_ugL","DIC_mgL")),
         color = factor(color, levels=c("PO4_ugL","TotalConc_ugL","Bluegreens_ugL","pH","DOC_mgL","co2_umolL","NH4_ugL","DIC_mgL")))
...
# New facet label names for sum_plot
fac.labs <- c("SRP (ug/L)","Phyto. (ug/L)","pH","DOC (mg/L)","CO2 (µmol/L)","NH4 (µg/L)","DIC_mgL")
names(fac.labs) <- c("PO4_ugL","Phyto","pH","DOC_mgL", "co2_umolL","NH4_ugL","DIC_mgL")
...
sum_plot4%>%
  ...
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  scale_color_manual(values = c("grey50","#3DB158","turquoise","blue","#8E4412","skyblue","magenta4","pink"))+
  facet_grid(rows = vars(name), cols = vars(Year), scales = "free", switch = "y",
             labeller = labeller(name = fac.labs))+
  theme_bw()+
  theme(strip.placement = "outside", legend.position = "none",
