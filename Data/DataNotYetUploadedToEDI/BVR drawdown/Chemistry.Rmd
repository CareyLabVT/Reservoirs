...
jpeg("./Figs/Abby_money_plot_biogeo.jpeg", res = 300, width = 6.5, height = 4, units = "in")
sum_plot3%>%
  mutate(show_points = ifelse(name == "thermo","n","y"))%>%
  ggplot(aes(x = DateTime, y = value, color = name))+
  geom_point(aes(alpha = show_points))+
...
sum_plot4 = chem_full%>%
  filter(Depth_m ==0.1)%>%
  select(DateTime, PO4_ugL,DOC_mgL,DIC_mgL,NO3NO2_ugL,NH4_ugL)%>%
  full_join(fluora_surface%>%
              filter(year(DateTime)%in% c(2021,2022)))%>%
  full_join(thermo_depth%>%mutate(DateTime = as.Date(Date))%>%select(-Date))%>%
  full_join(light%>%mutate(DateTime = as.Date(Date))%>%select(-Date))%>%
  ...
  full_join(ghg%>%mutate(DateTime = as.Date(DateTime)))%>%
  select(DateTime, PO4_ugL,DOC_mgL,TotalConc_ugL,pH,NH4_ugL,co2_umolL,ch4_umolL,Bluegreens_ugL)%>%
  pivot_longer(cols = c(PO4_ugL,DOC_mgL,TotalConc_ugL,pH,co2_umolL,ch4_umolL,NH4_ugL,Bluegreens_ugL))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime),
  ...
  mutate(name = factor(name, levels=c("PO4_ugL","Phyto","pH","DOC_mgL","ch4_umolL","co2_umolL","NH4_ugL")),
         color = factor(color, levels=c("PO4_ugL","TotalConc_ugL","Bluegreens_ugL","pH","DOC_mgL","ch4_umolL","co2_umolL","NH4_ugL")))
...
# New facet label names for sum_plot
fac.labs <- c("SRP (ug/L)", "Phyto.\n(ug/L)", "pH", "DOC\n(mg/L)", "CO2\n(µmol/L)", "CH4\n(µmol/L)", "NH4 (µg/L)")
names(fac.labs) <- c("PO4_ugL", "Phyto", "pH", "DOC_mgL", "co2_umolL", "ch4_umolL", "NH4_ugL")

jpeg("./Figs/Abby_money_plot_doc.jpeg", res = 300, width = 6.5, height = 5.6, units = "in")
sum_plot4%>%
  ggplot(aes(x = DateTime, y = value, color = color))+
  geom_point()+
  geom_line()+
  geom_vline(aes(xintercept = max_date, color = color), data = maxes, lwd = 2)+
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  scale_color_manual(values = c("grey50","darkgreen","turquoise","blue","#8E4412","skyblue","pink","magenta4"))+
  facet_grid(rows = vars(name), cols = vars(Year), scales = "free", switch = "y",
             labeller = labeller(name = fac.labs))+
  theme_bw()+
  theme(strip.placement = "outside", legend.position = "none",
...
sum_plot5 = chem_full%>%
  ...
  mutate(name = factor(name, levels=c("Turbidity_NTU", "Zeu", "Cmax_depth", "PeakWidth")),
         color = factor(color, levels=c("Turbidity_NTU", "Zeu","CmaxDepth_TotalConc_ugL", "CmaxDepth_Bluegreens_ugL", "PeakWidth_TotalConc_m", "PeakWidth_Bluegreens_m")))
...
# New facet label names for sum_plot
fac.labs <- c("Turbidity_NTU", "Zeu", "Cmax_depth", "PeakWidth")
names(fac.labs) <- c("Turbidity_NTU", "Zeu", "Cmax_depth", "PeakWidth")

jpeg("./Figs/Abby_money_plot_phytos.jpeg", res = 300, width = 6.5, height = 5.6, units = "in")
sum_plot5%>%
  ggplot(aes(x = DateTime, y = value, color = color))+
  geom_point()+
  geom_line()+
  ...
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  scale_color_manual(values = c("grey20","goldenrod","darkgreen","turquoise","darkgreen","turquoise"))+
  facet_grid(rows = vars(name), cols = vars(Year), scales = "free", switch = "y",
             labeller = labeller(name = fac.labs))+
  theme_bw()+
  theme(strip.placement = "outside", legend.position = "none",
