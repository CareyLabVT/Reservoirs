...
inUrl1  <- "https://pasta-s.lternet.edu/package/data/eml/edi/1025/6/509f39850b6f95628d10889d66885b76" 
infile1 <- tempfile()
...
sum_plot2 = chem_full%>%
  ...
  full_join(ctd%>%mutate(DateTime = as.Date(Date))%>%select(-Date))%>%
...
sum_plot3 = chem_full%>%
  ...
  pivot_longer(cols = c(TFe_mgL,TMn_mgL,SFe_mgL,SMn_mgL,DOsat_percent,thermo))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime),
  ...
  mutate(name = factor(name, levels=c("thermo","DOsat_percent","Fe_mgL","Mn_mgL")),
         color = factor(color, levels=c("thermo","DOsat_percent","TFe_mgL","TMn_mgL","SFe_mgL","SMn_mgL"),
                        labels=c("thermo","DOsat_percent","TFe_mgL","TMn_mgL","SFe_mgL","SMn_mgL")),
...
horiz_line = data.frame(name=rep("DOsat_percent",2),y = rep(100,2),Year=c(2021,2022))%>%
  mutate(name = factor(name, levels=c("thermo","DOsat_percent","TFe_mgL","TMn_mgL","SFe_mgL","SMn_mgL")))
sum_plot3%>%
  mutate(show_points = ifelse(name == "thermo","n","y"))%>%
  ggplot()+
  #geom_hline(aes(yintercept = y),data=horiz_line, color = "grey20",alpha=0.3,size=1)+
...
my_colors = c("black","grey50","darkgreen","turquoise","blue","#8E4412","skyblue","magenta4","pink")
...
sum_plot4 = chem_full%>%
  ...
  pivot_longer(cols = c(TP_ugL, TN_ugL,PO4_ugL,DOC_mgL,TotalConc_ugL,co2_umolL,NH4_ugL,Bluegreens_ugL,TNTP))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime),
  ...
  mutate(name = factor(name, levels=c("Phosphorus","Nitrogen","TNTP","Phyto","DOC_mgL","co2_umolL")),
         color = factor(color, levels=c("TP_ugL","PO4_ugL","TN_ugL","NH4_ugL","TotalConc_ugL","Bluegreens_ugL","DOC_mgL","co2_umolL","TNTP"),
                        labels=c("TP","PO4","TN","NH4","Total phytoplankton", "Cyanobacteria", "DOC", "CO2", "TN:TP")))
...
sum_plot4%>%
  ...
  scale_linetype_manual(values = c("solid","11","solid","11","solid","11","solid","solid","solid"),
                        #breaks = c("TP_ugL","PO4_ugL","TN_ugL","NH4_ugL","TotalConc_ugL","Bluegreens_ugL"),
                        #labels = c("TP","PO4","TN","NH4","Total phytoplankton","Cyanobacteria")
                        )+
...
chem_full%>%
  filter(Depth_m ==0.1)%>%
  ggplot(aes(x=DateTime,y=NO3NO2_ugL/PO4_ugL))+
  geom_point()

chem_full%>%
  filter(Depth_m ==0.1)%>%
  ggplot(aes(x=DateTime,y=TN_ugL/TP_ugL))+
  geom_point()
...
thermo_depth = read.csv("./Data/thermocline_depth.csv")
min(thermo_depth$Date, na.rm=T)

light = read.csv("./Data/attenuation_calc.csv")
min(light$Date, na.rm=T)

ctd = read.csv("./Data/CTD at 0.1m.csv")%>%
  filter(hour(DateTime)>8,hour(DateTime)<17)
min(ctd$DateTime, na.rm=T)

ghg = read.csv("./Data/GHG at 0.1m.csv")
min(ghg$DateTime, na.rm=T)

metals = read.csv("./Data/Metals at 0.1m.csv")
min(metals$Date, na.rm=T)
...
horiz_line = data.frame(name=rep("DOsat_percent",5),y = rep(100,5),Year=2018:2022)%>%
  mutate(name = factor(name, levels=c("thermo","DOsat_percent","TFe_mgL","TMn_mgL")))
sum_plot3%>%
  mutate(show_points = ifelse(name == "thermo","n","y"))%>%
  ggplot(aes(x = DateTime, y = value, color = name))+
  ...
  force_panelsizes(rows = unit(.75, "in"),
                   cols = unit(2.5, "in"))
ggsave("./Figs/Abby_money_plot_biogeo_long.jpeg", dpi = 300, width = 25, height = 4)

sum_plot3%>%
  mutate(show_points = ifelse(name == "thermo","n","y"),
         year_class = ifelse(Year == 2022,"2022","2013-2021"),
         Year = as.factor(Year))%>%
  ggplot(aes(x = yday(DateTime), y = value, color = Year))+
  geom_hline(aes(yintercept = y),data=horiz_line, color = "grey50",alpha=0.3,size=1)+
  geom_point(aes(alpha = show_points), show.legend = F)+
  geom_line()+
  scale_alpha_manual(values = c(0,1))+
  geom_vline(aes(xintercept = yday(as.Date(max_date, origin="1970-01-01")), color = year_class), data = maxes, lwd = 2, alpha = 0.5)+
  geom_vline(data=lines,aes(xintercept = yday(as.Date(Dates, origin="1970-01-01")), lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  #scale_color_manual(values = c("darkblue","cyan4","lightsalmon","coral4"))+
  facet_grid(rows = vars(name), cols = vars(year_class), scales = "free", switch = "y",
             labeller = labeller(name = fac.labs))+
  theme_bw()+
  theme(strip.placement = "outside",
        strip.text.x = element_text(
  ...
  force_panelsizes(rows = unit(.75, "in"),
                   cols = unit(2.5, "in"))
ggsave("./Figs/Abby_money_plot_biogeo_allYears.jpeg", dpi = 300, width = 7, height = 4)
...
sum_plot4 = chem_full%>%
  ...
  full_join(ctd%>%mutate(DateTime = as.Date(Date))%>%select(-Date,-Reservoir))%>%
  full_join(ghg%>%mutate(DateTime = as.Date(DateTime)))%>%
  select(DateTime, TP_ugL, PO4_ugL,DOC_mgL,TotalConc_ugL,NH4_ugL,co2_umolL,Bluegreens_ugL)%>%
  pivot_longer(cols = c(TP_ugL, PO4_ugL,DOC_mgL,TotalConc_ugL,co2_umolL,NH4_ugL,Bluegreens_ugL))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime),
         ...
         name = ifelse(name%in%c("TotalConc_ugL","Bluegreens_ugL"),"Phyto",name),
         name = ifelse(name%in%c("TP_ugL","PO4_ugL"),"Phosphorus",name))%>%
  mutate(name = factor(name, levels=c("Phosphorus","Phyto","DOC_mgL","co2_umolL","NH4_ugL")),
         color = factor(color, levels=c("PO4_ugL","TP_ugL","TotalConc_ugL","Bluegreens_ugL","DOC_mgL","co2_umolL","NH4_ugL"),
                        labels=c("SRP (ug/L)","Total\nPhosphorus\n(ug/L)", "Total\nPhyto.\n(ug/L)", "Cyano\n(µg/L)", "DOC\n(mg/L)", "CO2\n(µmol/L)", "NH4 (µg/L)")))
...
# New facet label names for sum_plot
fac.labs <- c("Phosphorus\n(ug/L)", "Phyto.\n(ug/L)", "DOC\n(mg/L)", "CO2\n(µmol/L)", "NH4 (µg/L)")
names(fac.labs) <- c("Phosphorus", "Phyto", "DOC_mgL", "co2_umolL", "NH4_ugL")

my_colors = c("black","grey50","darkgreen","turquoise","#8E4412","skyblue","magenta4")
names(my_colors)=c("TP_ugL","PO4_ugL","TotalConc_ugL","Bluegreens_ugL","DOC_mgL","co2_umolL","NH4_ugL")
...
sum_plot4%>%
  ...
  force_panelsizes(rows = unit(.75, "in"),
                   cols = unit(2.5, "in"))
ggsave("./Figs/Abby_money_plot_doc_long.jpeg", dpi = 300, width = 25, height = 5)

# New facet label names for sum_plot
fac.labs <- c("Total\nPhosphorus\n(ug/L)", "SRP (ug/L)","Total\nPhyto.\n(ug/L)", "Cyano\n(µg/L)", "DOC\n(mg/L)", "CO2\n(µmol/L)", "NH4 (µg/L)")
names(fac.labs) <- c("TP_ugL", "PO4_ugL", "TotalConc_ugL","Bluegreens_ugL","DOC_mgL", "co2_umolL", "NH4_ugL")
sum_plot4%>%
  mutate(year_class = ifelse(Year == 2022,"2022","2013-2021"),
         Year = as.factor(Year),
         group = paste0(color,Year))%>%
  ggplot(aes(x = as.Date(yday(DateTime), origin = "2022-01-01"), y = value, color = year_class, group = group))+
  geom_point()+
  geom_line()+
  geom_point(data = .%>%filter(Year==2022))+
  geom_line(data = .%>%filter(Year==2022))+
  geom_vline(aes(xintercept = max_date, color = year_class), data = maxes, lwd = 2, alpha = 0.5)+
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  scale_color_manual(values = c("grey80","#0800FA")
                     )+
  facet_grid(rows = vars(color), 
             #cols = vars(year_class), 
  ...
  force_panelsizes(rows = unit(.75, "in"),
                   cols = unit(2.5, "in"))
ggsave("./Figs/Abby_money_plot_doc_allYears.jpeg", dpi = 300, width = 4, height = 7)
