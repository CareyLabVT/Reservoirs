...
sum_plot2 = chem_full%>%
  ...
  select(DateTime, PO4_ugL,DOC_mgL,Bluegreens_ugL,TotalConc_ugL,thermo,Zeu,Turbidity_NTU,SpCond_uScm,DO_mgL,NO3NO2_ugL,NH4_ugL, pH)%>%
  pivot_longer(cols = c(PO4_ugL,DOC_mgL,Bluegreens_ugL,TotalConc_ugL,thermo,Zeu,Turbidity_NTU,SpCond_uScm,DO_mgL,NO3NO2_ugL,NH4_ugL, pH))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime))%>%
  mutate(name = factor(name, levels=c("PO4_ugL","TotalConc_ugL","Bluegreens_ugL","pH","DOC_mgL","Turbidity_NTU","Zeu","thermo","SpCond_uScm","DO_mgL","NO3NO2_ugL","NH4_ugL")))
...
# New facet label names for sum_plot
fac.labs <- c("SRP (ug/L)","Total phyto. (ug/L)","Cyanobacteria (ug/L)","pH","DOC (mg/L)","Thermocline depth (m)","Euphotic zone (m)","Turbidity (NTU)","SpCond_uScm","DO_mgL","NO3NO2_ugL","NH4_ugL")
names(fac.labs) <- c("PO4_ugL","TotalConc_ugL","Bluegreens_ugL","pH","DOC_mgL","thermo", "Zeu","Turbidity_NTU","SpCond_uScm","DO_mgL","NO3NO2_ugL","NH4_ugL")

jpeg("./Figs/Abby_money_plot_with_thermo.jpeg", res = 300, width = 6.5, height = 10, units = "in")
sum_plot2%>%
...
sum_plot3 = chem_full%>%
  ...
  full_join(light%>%mutate(DateTime = as.Date(Date))%>%select(-Date))%>%
  full_join(ctd%>%mutate(DateTime = as.Date(DateTime))%>%select(-Date))%>%
  select(DateTime, thermo,Turbidity_NTU,Zeu,DO_mgL)%>%
  pivot_longer(cols = c(thermo,Turbidity_NTU,Zeu,DO_mgL))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime))%>%
  mutate(name = factor(name, levels=c("thermo","Turbidity_NTU","Zeu","DO_mgL")))
...
# New facet label names for sum_plot
fac.labs <- c("Thermocline\ndepth (m)","Euphotic\nzone (m)","Surface DO\n(mgL)","Turbidity (NTU)")
names(fac.labs) <- c("thermo", "Zeu","DO_mgL","Turbidity_NTU")

jpeg("./Figs/Abby_money_plot_biogeo.jpeg", res = 300, width = 6.5, height = 5.6, units = "in")
sum_plot3%>%
  mutate(show_points = ifelse(name == "thermo","n","y"))%>%
  ggplot(aes(x = DateTime, y = value, color = name))+
  geom_point(aes(alpha = show_points))+
...
sum_plot4 = chem_full%>%
  ...
  full_join(thermo_depth%>%mutate(DateTime = as.Date(Date))%>%select(-Date))%>%
  full_join(light%>%mutate(DateTime = as.Date(Date))%>%select(-Date))%>%
  full_join(ctd%>%mutate(DateTime = as.Date(DateTime))%>%select(-Date,-Reservoir))%>%
  select(DateTime, PO4_ugL,DOC_mgL,Bluegreens_ugL,TotalConc_ugL,pH,NH4_ugL)%>%
  pivot_longer(cols = c(PO4_ugL,DOC_mgL,Bluegreens_ugL,TotalConc_ugL,pH,NH4_ugL))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime))%>%
  mutate(name = factor(name, levels=c("PO4_ugL","TotalConc_ugL","Bluegreens_ugL","pH","DOC_mgL","NH4_ugL")))
...
# New facet label names for sum_plot
fac.labs <- c("SRP (ug/L)","Total phyto.\n(ug/L)","Cyanobacteria\n(ug/L)","pH","DOC (mg/L)","NH4 (µg/L)")
names(fac.labs) <- c("PO4_ugL","TotalConc_ugL","Bluegreens_ugL","pH","DOC_mgL", "NH4_ugL")

jpeg("./Figs/Abby_money_plot_extend.jpeg", res = 300, width = 6.5, height = 5.6, units = "in")
sum_plot4%>%
  ggplot(aes(x = DateTime, y = value, color = name))+
  geom_point()+
  geom_line()+
  ...
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  scale_color_manual(values = c("#202C39","#2DB138","#00BED0","black","#8E4416","#BA9800"))+
  facet_grid(rows = vars(name), cols = vars(Year), scales = "free", switch = "y",
             labeller = labeller(name = fac.labs))+
  theme_bw()+
  theme(strip.placement = "outside", legend.position = "none",
