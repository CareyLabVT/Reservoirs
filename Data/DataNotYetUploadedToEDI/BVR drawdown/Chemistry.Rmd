...
sum_plot4 = chem_full%>%
  ...
  mutate(name = factor(name, levels=c("Phosphorus","Phyto","DOC_mgL","co2_umolL","NH4_ugL")),
         color = factor(color, levels=c("TP_ugL","PO4_ugL","TotalConc_ugL","Bluegreens_ugL","DOC_mgL","co2_umolL","NH4_ugL"),
                        labels=c("TP_ugL","PO4_ugL","TotalConc_ugL","Bluegreens_ugL","DOC_mgL","co2_umolL","NH4_ugL")))
...
sum_plot4%>%
  ...
  ggplot(aes(x = yday(DateTime), y = value, color = Year, group = group))+
  geom_point()+
  geom_line()+
  geom_vline(aes(xintercept = yday(as.Date(max_date, origin = "1970-01-01")), color = as.factor(Year)), data = maxes, lwd = 2, alpha = 0.5)+
  geom_vline(data=lines,aes(xintercept = yday(as.Date(Dates, origin = "1970-01-01")), lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  #scale_color_manual(breaks = c("TotalConc_ugL","Bluegreens_ugL"),
  #                   labels = c("Total (µg/L)","Cyanobacteria (µg/L)"),
  ...
  force_panelsizes(rows = unit(.75, "in"),
                   cols = unit(2.5, "in"))
ggsave("./Figs/Abby_money_plot_doc_allYears.jpeg", dpi = 300, width = 7, height = 5)
