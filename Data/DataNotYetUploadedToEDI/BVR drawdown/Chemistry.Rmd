...
sum_plot3 = chem_full%>%
  ...
  full_join(metals%>%mutate(DateTime = as.Date(Date))%>%select(-Date,-Date_22))%>%
  select(DateTime, Turbidity_NTU,Zeu,DO_mgL,TFe_mgL,TMn_mgL)%>%
  pivot_longer(cols = c(Turbidity_NTU,Zeu,DO_mgL,TFe_mgL,TMn_mgL))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime))%>%
  mutate(name = factor(name, levels=c("Turbidity_NTU","Zeu","DO_mgL","TFe_mgL","TMn_mgL")))
...
# New facet label names for sum_plot
fac.labs <- c("Euphotic\nzone (m)","Surface DO\n(mgL)","Turbidity (NTU)","Total Fe\n(mg/L)","Total Mn\n(mg/L)")
names(fac.labs) <- c( "Zeu","DO_mgL","Turbidity_NTU","TFe_mgL","TMn_mgL")
...
sum_plot4 = chem_full%>%
  ...
  full_join(ghg%>%mutate(DateTime = as.Date(DateTime)))%>%
  select(DateTime, PO4_ugL,DOC_mgL,TotalConc_ugL,pH,NH4_ugL,co2_umolL)%>%
  pivot_longer(cols = c(PO4_ugL,DOC_mgL,TotalConc_ugL,pH,co2_umolL,NH4_ugL))%>%
  filter(!is.na(value),
         month(DateTime)>4,
         month(DateTime)<9)%>%
  mutate(Year = year(DateTime))%>%
  mutate(name = factor(name, levels=c("PO4_ugL","TotalConc_ugL","pH","DOC_mgL","co2_umolL","NH4_ugL")))
...
# New facet label names for sum_plot
fac.labs <- c("SRP (ug/L)","Total phyto.\n(ug/L)","pH","DOC (mg/L)","CO2 (µmol/L)","NH4 (µg/L)")
names(fac.labs) <- c("PO4_ugL","TotalConc_ugL","pH","DOC_mgL", "co2_umolL","NH4_ugL")
...
sum_plot4%>%
  ...
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  scale_color_manual(values = c("grey50","#3DB158","blue","#8E4412","skyblue","magenta4"))+
  facet_grid(rows = vars(name), cols = vars(Year), scales = "free", switch = "y",
             labeller = labeller(name = fac.labs))+
  theme_bw()+
  theme(strip.placement = "outside", legend.position = "none",
