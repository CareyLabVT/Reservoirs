...
metals = metals%>%
  ...
  filter(month(Sample_Date)>=4,month(Sample_Date)<=8)
...
metals$Date_22 = metals$Sample_Date
year(metals$Date_22)=2022
metals$Year = as.factor(year(metals$Sample_Date))
...
metals%>%
  ...
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  geom_point(aes(color = Year,y = TFe_ppb))+
  theme_bw()+
  ylab("Surface iron (ppb)")+
  theme(axis.title.x = element_blank())
dev.off()

summary(as.factor(metals$Reservoir))
```
...
sensor_depth = sensor%>%
  ...
  filter(DateTime>=min(metals$Sample_Date),
         DateTime<=max(metals$Sample_Date),
         WaterLevel>7.5)%>%
  arrange(DateTime)%>%
...
metals = metals%>%
  left_join(sensor_depth%>%rename(Sample_Date=Date))%>%select(-max)%>%
