...
jpeg("./Figs/Surface Mn (ppb).jpeg", res = 300, width = 6, height = 4, units = "in")
metals%>%
  filter(year(Date)%in%c(2021,2022),
         Reservoir == "BVR",
         Depth_m == 0.1,
  ...
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  geom_point(aes(color = Year,y = TMn_mgL))+
  theme_bw()+
  ylab("Surface Mn (mg/L)")+
  theme(axis.title.x = element_blank())
dev.off()

jpeg("./Figs/Surface Mn ST ratio.jpeg", res = 300, width = 6, height = 4, units = "in")
metals%>%
  filter(year(Date)%in%c(2021,2022),
         Reservoir == "BVR",
         Depth_m == 0.1,
  ...
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dashed"))+
  geom_point(aes(color = Year,y = SMn_mgL/TMn_mgL))+
  theme_bw()+
  ylab("Surface SMn/TMn ratio")+
  theme(axis.title.x = element_blank())
dev.off()
...
write.csv(export,"./Data/Metals at 0.1m.csv",row.names=F)
```



BVR metals heatmaps with sensor data (elevation rather than depth)
```{r}
#metals = read_csv("./Data/Drawdown Metals_updated.csv")%>%
#  filter(Reservoir == "BVR")%>%
...
Weir figures - HASN'T BEEN UPDATED WITH NEW METALS EDI PACKAGE
```{r}
...
metals%>%
  ...
  ggplot(aes(x = Date_22, y = TFe_ppb,color = Year))+
  geom_point()
```
