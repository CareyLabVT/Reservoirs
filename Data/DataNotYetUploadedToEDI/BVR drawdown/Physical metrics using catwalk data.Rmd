...
p = interp%>%
  ...
  geom_line(data = thermo_raw%>%
              mutate(Year=year(Date))%>%
              filter(Year%in%c(2021,2022)), aes(x = Date, y = Reservoir_depth-thermo, lty = "Thermocline"))+
  geom_hline(aes(yintercept = 6.6, lty = "Out-take pipe"))+
  ...
  geom_vline(data=lines,aes(xintercept = Dates, lty = status), show.legend = F)+
  scale_linetype_manual(values = c("solid","dotted","solid","dashed"),limits = c("Thermocline","Out-take pipe","final","init"),  name= NULL)+
