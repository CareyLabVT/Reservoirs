...
surf = cat_long%>%
  mutate(Date = as.Date(DateTime),
         Year = year(Date))%>%
  group_by(Date, Year)%>%
  summarize(Reading = Reading[which.min(Sensor_depth)],
...
bottom = cat_long%>%
  mutate(Date = as.Date(DateTime),
         Year = year(Date))%>%
  group_by(Date, Year)%>%
  summarize(Reading = Reading[which.max(Sensor_depth)],
            Sensor_depth = 0)
...
depths = cat_long%>%
  mutate(Date = as.Date(DateTime))%>%
  group_by(Date)%>%
  summarize(Depth_m = mean(Reservoir_depth))%>%
  mutate(Year = year(Date))
...
depths = cat_long%>%
  mutate(Date = as.Date(DateTime))%>%
  group_by(Date)%>%
  summarize(Reservoir_depth = mean(Reservoir_depth))%>%
  mutate(Year = year(Date))
...
vol_sum = flexible_bathy%>%
  group_by(Date)%>%
  summarize(depth_above_water = Depth_m[Depth_m<0],
...
meta_ratio = flexible_bathy%>%
  left_join(thermo_raw%>%select(-Reservoir_depth))%>%
  filter(!is.na(thermo))%>%
  group_by(Date)%>%
  summarize(depth_above_water = Depth_m[Depth_m<0],
...
meta_ratio%>%
  ...
  filter(!Year==2020,
         month(Date)>4,
         month(Date)<11)%>%
  select(Date_22, Year, epi_volume,hypo_volume)%>%
