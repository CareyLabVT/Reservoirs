...
```{r format time, echo=FALSE, warning=FALSE}
# Format time
...
ec2 %>% group_by(year = year(datetime), month = factor(month.abb[month(datetime)], levels = c("Apr", "May", "Jun",
                                                                                                 ...
                                                                                                 'Dec', 'Jan', 'Feb', 'Mar')), 
                    hour = hour(datetime)) %>% 
  summarise(air_temperature = mean(air_temperature_k, na.rm = TRUE), .groups = 'drop') %>% 
...
# For data period: EC = Met*0.562065+0.095610
...
########################################################################
# Count available data before gap filling
...
###########################################################################
# Gap filling - using periods of similar met.
# Following Eddy Proc
...
fcr_gf_mean <- fcr_gf %>% 
  mutate(DateTime = format(as.POSIXct(DateTime, format = "%Y-%m-%d"),"%Y-%m-%d")) %>% 
...
fcr_gf %>% ggplot() +
  ...
  geom_line(aes(DateTime, NEE_U05_f), col = 'blue', alpha = 0.4)+
  theme_bw() +
