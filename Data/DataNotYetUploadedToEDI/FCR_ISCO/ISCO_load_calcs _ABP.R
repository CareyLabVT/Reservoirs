...
folder<-'./Data/DataNotYetUploadedToEDI/FCR_ISCO/'

#Load data
Discharge <-alls
#Discharge <- read_csv(paste0(folder, "2019_2022_ISCO_FCRWEIR_WaterLevel.csv"))
...
Discharge_v <- Discharge %>% mutate(Flow_cms = 2.391*(WaterLevel_m^2.5))  
...
Discharge_r <- Discharge %>% mutate(Flow_cms = ifelse(WaterLevel_m > 0.275, 
...
ggplot()+
  geom_scattermore(data=Discharge_r, aes(x=DateTime, y=Flow_cms))+
  geom_scattermore(data=Discharge_v, aes(x=DateTime, y=Flow_cms), color="red")


par(mfrow=c(1,2))
plot(Discharge_v$DateTime,Discharge_v$Flow_cms, type='l') 
plot(Discharge_r$DateTime,Discharge_r$Flow_cms) 
dev.off()

plot(Discharge_r$DateTime,Discharge_r$Flow_cms)
points(Discharge_v$DateTime,Discharge_v$Flow_cms,col="red")
...
Carey_Q$DateTime = ymd_hms(Carey_Q$DateTime)

# Merge ISCO Q with Carey Q
All_Q_v = merge(Discharge_v,Carey_Q2, by = "DateTime", all.x = TRUE)
...
plot(All_Q_v$DateTime,All_Q_v$Flow_cms, type="l")
points(All_Q_v$DateTime,All_Q_v$VT_Flow_cms, col="red")
points(All_Q_v$DateTime, All_Q_v$WVWA_Flow_cms, col="blue")
...
# Read in weir metals files
met<- read_csv(paste0(folder,"/Data/ISCO_metals_samples_2019_2020.csv"))
...
dt1 <-read_csv(infile1)%>%
  ...
  group_by(DateTime)%>%
  summarise(across(c(TN_ugL, TP_ugL), mean, na.rm=T))%>%
...
met_avg<-met%>%
  ...
  group_by(DateTime)%>%
  summarise(across(c(TFe_mgL, TMn_mgL), mean, na.rm=T))%>%
  mutate(Date=as.Date(DateTime))
  

met_dis<-merge(Discharge_v, met_avg, by="DateTime", all=T)
  
nv<-merge(Date,met_avg, by="Date", all=T)

# If the DateTime from the download is an NA then use the DateTime from metals sample
nv$DateTime.x<- coalesce(nv$DateTime.x, nv$DateTime.y)

# clean up the data frame
meta<-nv%>%
  select(DateTime.x,Download, TFe_mgL, TMn_mgL, Date)%>%
  dplyr::rename("DateTime"="DateTime.x")

# merge with the nutrients 

all_samp=merge(meta, dt1, by="Date", all=T)

all_samples<-all_samp%>%
  select(DateTime.x, TFe_mgL, TMn_mgL, TN_ugL, TP_ugL, Download)%>%
  dplyr::rename("DateTime"="DateTime.x")%>%
  mutate(Date=as.Date(DateTime))


# Get the start date/ when the ISCO was deployed for each year

startdate<-waterlevel3%>%
...
all_samples2=merge(all_samples, lastdate, by="Date", all=T)

#
# If the DateTime from the download is an NA then use the DateTime from metals sample
all_samples2$DateTime.x<- coalesce(all_samples2$DateTime.x, all_samples2$DateTime.y)

all_samples3<-all_samples2%>%
  select(Date, DateTime.x, TFe_mgL, TMn_mgL, TN_ugL, TP_ugL, Download, Start_date)%>%
  dplyr::rename("DateTime"="DateTime.x")
...
time = data.frame(all_samples3$DateTime, all_samples3$DateTime, all_samples3$Start_date)
...
# create the lists of discharge based on the sample durationof deployments. When the ISCO started collecting the sample
# until it ended. 
sample = list()
  for(i in 1:nrow(time2)){
  sample[[i]] = Discharge_v[Discharge_v$DateTime>time2[i,1] & Discharge_v$DateTime<time2[i,2],]
}
...
for(i in 1:nrow(time2)){
  time2$cum_v[i] = sum(sample[[i]]$cum_flow, na.rm = T)
}

# read in csv with cummulative flow for each sampling time period

flow<-read_csv(paste0(folder, "ISCO Sample Data.csv"))%>%
  mutate(DateTime=mdy_hm(DateTime),
         Date=as.Date(DateTime))

check<-time2%>%
  mutate(Date=as.Date(End_time))%>%
  merge(., flow, by="Date", all=T)%>%
  select(Start_time, End_time, Date, cum_v, Cummulative_water_volume)
