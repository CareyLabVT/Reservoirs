...
# Set the WD 
#setwd("C:/BVR_BVR Metals Data/EDI")

# load in metal data from the season (the excel sheet you worked on)
BVR <- read_csv("Metals_2022 - Sheet1.csv") %>% filter(Reservoir=="BVR") %>% na.omit(BVR)
#convert characters to dates/numbers
BVR$TFe_mgL<-as.numeric(BVR$TFe_mgL)
#eliminate all rows where TFe is missing
BVR<-BVR[which(!is.na(BVR$TFe_mgL)),]
BVR$TMn_mgL<-as.numeric(BVR$TMn_mgL)
#eliminate all rows where TMn is missing
BVR<-BVR[which(!is.na(BVR$TMn_mgL)),]

BVR$SFe_mgL<-as.numeric(BVR$SFe_mgL)
BVR<-BVR[which(!is.na(BVR$SFe_mgL)),]

BVR$SMn_mgL<-as.numeric(BVR$SMn_mgL)
BVR<-BVR[which(!is.na(BVR$SMn_mgL)),]


# Filter to include desired reservoir (BVR), site (50), and year (2022)

#run this one once all entries have date and time
#BVR$DateTime <- ymd_hms(BVR$DateTime)

BVR$DateTime<-as.Date(BVR$DateTime, "%Y-%m-%d")

BVR <- BVR %>% filter(Reservoir=="BVR") %>%
  filter(Site==50) %>%
...
TFe <- select(BVR, DateTime, Depth_m, TFe_mgL) 
TMn <- select(BVR, DateTime, Depth_m, TMn_mgL)
SFe <- select(BVR, DateTime, Depth_m, SFe_mgL)
SMn <- select(BVR, DateTime, Depth_m, SMn_mgL)
DateTime_u <- as.data.frame(unique(date(ymd(BVR$DateTime))))
colnames(DateTime_u) <- "DateTime"
DateTime_x <- date(ymd(BVR$DateTime))       
...
#Total Iron
png('BVR TFe Heatmap 2022_test.png', width = 15, height = 12, units = 'in', res = 300)
p1 <- ggplot(interp_TFe_f, aes(x=DateTime_i, y=y))+
  ...
  labs(x = "Date", y = "Depth (m)", title = "BVR Total Iron (mg/L) 2022",fill=expression('(mg/L)'))+
...
#Total Mn
png('BVR TMn Heatmap 2022.png', width = 15, height = 12, units = 'in', res = 300)
p2 <- ggplot(interp_TMn_f, aes(x=DateTime_i, y=y))+
  geom_raster(aes(fill=z), interpolate = TRUE)+
  scale_y_reverse()+
  geom_point(data = DateTime_u, aes(x=DateTime, y=0, z=NULL), pch = 25, size = 1.5, color = "white", fill = "black")+
  scale_fill_gradientn(colours = blue2green2red(60), na.value="gray")+
  labs(x = "Date", y = "Depth (m)", title = "BVR Total Manganese (mg/L) 2022",fill=expression('(mg/L)'))+
  theme_black()
print(p2)
dev.off()

#Soluble Fe
png('BVR SFe Heatmap 2022.png', width = 15, height = 12, units = 'in', res = 300)
p3 <- ggplot(interp_SFe_f, aes(x=DateTime_i, y=y))+
  geom_raster(aes(fill=z), interpolate = TRUE)+
  scale_y_reverse()+
  geom_point(data = DateTime_u, aes(x=DateTime, y=0, z=NULL), pch = 25, size = 1.5, color = "white", fill = "black")+
  scale_fill_gradientn(colours = blue2green2red(60), na.value="gray")+
  labs(x = "Date", y = "Depth (m)", title = "BVR Soluble Iron (mg/L) 2022",fill=expression('(mg/L)'))+
  theme_black()
print(p3)
dev.off()

#Soluble Mn
png('BVR SMn Heatmap 2022.png', width = 15, height = 12, units = 'in', res = 300)
p3 <- ggplot(interp_SMn_f, aes(x=DateTime_i, y=y))+
  geom_raster(aes(fill=z), interpolate = TRUE)+
  scale_y_reverse()+
  geom_point(data = DateTime_u, aes(x=DateTime, y=0, z=NULL), pch = 25, size = 1.5, color = "white", fill = "black")+
  scale_fill_gradientn(colours = blue2green2red(60), na.value="gray")+
  labs(x = "Date", y = "Depth (m)", title = "BVR Soluble Manganese (mg/L) 2022",fill=expression('(mg/L)'))+
  theme_black()
print(p3)
dev.off()
...
#Soluble Mn
png('BVR SMn Heatmap 2022 white.png', width = 15, height = 12, units = 'in', res = 300)
p4 <- ggplot(interp_SMn_f, aes(x=DateTime_i, y=y))+
  ...
  labs(x = "Date", y = "Depth (m)", title = "BVR Soluble Manganese (mg/L) 2021",fill=expression(''))+
  theme_new()
print(p4)
