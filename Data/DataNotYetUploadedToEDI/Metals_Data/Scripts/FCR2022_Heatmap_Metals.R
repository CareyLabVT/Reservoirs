...
FCR <- read_csv("Metals_2022 - Sheet1.csv") %>% na.omit(FCR)
#convert characters to dates/numbers
FCR$TFe_mgL<-as.numeric(FCR$TFe_mgL)
#eliminate all rows where TFe is missing
FCR<-FCR[which(!is.na(FCR$TFe_mgL)),]
FCR$TMn_mgL<-as.numeric(FCR$TMn_mgL)
#eliminate all rows where TMn is missing
FCR<-FCR[which(!is.na(FCR$TMn_mgL)),]

FCR$SFe_mgL<-as.numeric(FCR$SFe_mgL)
FCR<-FCR[which(!is.na(FCR$SFe_mgL)),]

FCR$SMn_mgL<-as.numeric(FCR$SMn_mgL)
FCR<-FCR[which(!is.na(FCR$SMn_mgL)),]


# Filter to include desired reservoir (FCR), site (50), and year (2021)

#run this one once all entries have date and time
##FCR$DateTime <- mdy_hms(FCR$DateTime)

FCR$DateTime<-as.Date(FCR$DateTime, "%Y-%m-%d")
...
DateTime_u <- as.data.frame(unique(date(ymd(FCR$DateTime))))
colnames(DateTime_u) <- "DateTime"
DateTime_x <- date(ymd(FCR$DateTime))       
...
#Total Iron
png('FCR TFe Heatmap 2022_test.png', width = 15, height = 12, units = 'in', res = 300)
p1 <- ggplot(interp_TFe_f, aes(x=DateTime_i, y=y))+
  ...
  labs(x = "Date", y = "Depth (m)", title = "FCR Total Iron (mg/L) 2022",fill=expression('(mg/L)'))+
...
#Total Mn
png('FCR TMn Heatmap 2022.png', width = 15, height = 12, units = 'in', res = 300)
p2 <- ggplot(interp_TMn_f, aes(x=DateTime_i, y=y))+
  geom_raster(aes(fill=z), interpolate = TRUE)+
  scale_y_reverse()+
  geom_point(data = DateTime_u, aes(x=DateTime, y=0, z=NULL), pch = 25, size = 1.5, color = "white", fill = "black")+
  scale_fill_gradientn(colours = blue2green2red(60), na.value="gray")+
  labs(x = "Date", y = "Depth (m)", title = "FCR Total Manganese (mg/L) 2022",fill=expression('(mg/L)'))+
  theme_black()
print(p2)
dev.off()

#Soluble Fe
png('FCR SFe Heatmap 2022.png', width = 15, height = 12, units = 'in', res = 300)
p3 <- ggplot(interp_SFe_f, aes(x=DateTime_i, y=y))+
  geom_raster(aes(fill=z), interpolate = TRUE)+
  scale_y_reverse()+
  geom_point(data = DateTime_u, aes(x=DateTime, y=0, z=NULL), pch = 25, size = 1.5, color = "white", fill = "black")+
  scale_fill_gradientn(colours = blue2green2red(60), na.value="gray")+
  labs(x = "Date", y = "Depth (m)", title = "FCR Soluble Iron (mg/L) 2022",fill=expression('(mg/L)'))+
  theme_black()
print(p3)
dev.off()

#Soluble Mn
png('FCR SMn Heatmap 2022.png', width = 15, height = 12, units = 'in', res = 300)
p3 <- ggplot(interp_SMn_f, aes(x=DateTime_i, y=y))+
  geom_raster(aes(fill=z), interpolate = TRUE)+
  scale_y_reverse()+
  geom_point(data = DateTime_u, aes(x=DateTime, y=0, z=NULL), pch = 25, size = 1.5, color = "white", fill = "black")+
  scale_fill_gradientn(colours = blue2green2red(60), na.value="gray")+
  labs(x = "Date", y = "Depth (m)", title = "FCR Soluble Manganese (mg/L) 2022",fill=expression('(mg/L)'))+
  theme_black()
print(p3)
dev.off()
...
#Soluble Mn
png('FCR SMn Heatmap 2022 white.png', width = 15, height = 12, units = 'in', res = 300)
p4 <- ggplot(interp_SMn_f, aes(x=DateTime_i, y=y))+
  ...
  labs(x = "Date", y = "Depth (m)", title = "FCR Soluble Manganese (mg/L) 2021",fill=expression(''))+
  theme_new()
print(p4)
