...
#"2017-08-27","2017-09-03" have a few oxygen values but mostly NA
```
...
ctd_flagged = ctd %>% #Add flags
  ...
  mutate(
    ...
    Flag_DO = ifelse(Date > as.Date("2021-06-26"),2, Flag_DO), #Oxygen sensor reading too high in the hypolimnion and too low at the surface
    ...
    Flag_DO_pSat = ifelse(Date > as.Date("2021-06-26"),2, Flag_DO_pSat), #Oxygen sensor reading too high in the hypolimnion and too low at the surface
    ...
    #ORP
    ORP_mV = ifelse(Date > as.Date("2021-01-01"),NA,ORP_mV), #ORP sensor was capped in 2021 because pH sensor was broken
    Flag_ORP = ifelse(Date > as.Date("2021-01-01"),2,Flag_ORP),
...
ctd_flagged$Date[year(ctd_flagged$Date) %in% c(2020,2021)] = ctd_flagged$Date[year(ctd_flagged$Date) %in% c(2020,2021)]+hours(13)
...
write.csv(CTD_fix,"CTD_final_2013_2021.csv", row.names = FALSE)
```
...
library(plotly)
qaqc = CTD_fix
qaqc = read.csv("CTD_final_2013_2021.csv")%>%
  mutate(Date = as_datetime(Date))
qaqc_plotly = qaqc%>%
  filter(Reservoir == "FCR", Site == 50, year(Date)%in%c(2020))%>%
  sample_frac(.1)%>%
  ggplot(aes(x = Date, y = Depth_m, color = ORP_mV))+
