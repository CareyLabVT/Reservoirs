...
do <- qaqc%>%
  ...
  group_by(round(Depth_m,1))%>%
  slice(which.min(Depth_m))
do_interp <- interp2xyz(interp(do$Date,do$Depth_m,do$DO_mgL,xo = seq(min(do$Date), max(do$Date), by = 1),yo = seq(min(do$Depth_m), max(do$Depth_m), by = .1), duplicate = "strip"),data.frame = T)
summary(do)
