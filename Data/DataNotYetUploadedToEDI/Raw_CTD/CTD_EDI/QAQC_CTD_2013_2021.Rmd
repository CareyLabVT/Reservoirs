...
#"2017-08-27","2017-09-03" have a few oxygen values but mostly NA. Current flag is 1 (sample not taken, so I will leave the remaining data)
```
...
CTD_fix=final%>%
  ...
  mutate(
    ...
    Flag_pH = ifelse((month(Date)==6&year(Date)==2019)|
              ...
              (month(Date)==2&year(Date)==2019),8,Flag_pH), #Flag suspect values
    
    pH = ifelse(as.Date(Date) %in% as.Date(c("2019-07-08","2018-12-06")),NA,pH), #pH outside of reasonable range
...
qaqc_plotly = qaqc%>%
  filter(Reservoir == "FCR", Site == 50, year(Date)%in%c(2017:2020))%>%
  sample_frac(.1)%>%
  ggplot(aes(x = Date, y = Depth_m, color = pH, shape = as.factor(Flag_pH)))+
