...
ctd_flagged = ctd %>% #Add flags
  ...
  mutate(
    ...
    Flag_Cond = ifelse(Date %in% as.POSIXct(c("2022-04-12 06:59:17")),2,Flag_Cond),
    ...
    Flag_SpecCond = ifelse(Date %in% as.POSIXct(c("2022-04-12 06:59:17")),2,Flag_SpecCond),
...
as_datetime("2022-02-24 08:01:48 CST")-as_datetime("2022-02-24 10:02:00 EDT")
as_datetime("2022-11-08 08:09:27 CST")-as_datetime("2022-11-08 10:10:00 EST")
#~2 hours
...
CTD_fix_renamed = CTD_fix%>% #Renaming flag columns in 2022
  ...
  select(Reservoir, Site, DateTime, Depth_m, Temp_C, DO_mgL, DOsat_percent, Cond_uScm, SpCond_uScm, Chla_ugL, Turbidity_NTU, pH, ORP_mV, PAR_umolm2s, DescRate_ms, Flag_Temp_C, Flag_DO_mgL, Flag_DOsat_percent, Flag_Cond_uScm, Flag_SpCond_uScm, Flag_Chla_ugL, Flag_Turb_NTU, Flag_pH, Flag_ORP_mV, Flag_PAR_umolm2s, Flag_DescRate_ms)
...
qaqc_plotly = qaqc%>%
  filter(Reservoir == "FCR", Site == 50, year(Date)%in%c(2020,2021,2022),Depth_m<9)%>%
  sample_frac(.1)%>%
  ggplot(aes(x = Date, y = Depth_m, color = Temp_C, shape = as.factor(Flag_Temp)))+
