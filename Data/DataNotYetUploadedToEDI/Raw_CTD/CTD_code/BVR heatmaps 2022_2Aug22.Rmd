...
df.final = df.final.raw%>%
  ...
  mutate(Turb_NTU=ifelse(Turb_NTU>10&Depth_m<0.3,NA,Turb_NTU)) #Sediment got mixed up in early 2022
...
for(var in vars){
  ...
  p = interp%>%
    ...
    scale_fill_gradientn(colours = if(var%in%c("DO_mgL","DO_pSat","ORP_mV")){rev(color_freya)}else{color_freya}, na.value="white", name = var)+
