...
df.final = df.final.raw%>%
  ...
  mutate(Turbidity_NTU=ifelse(Turbidity_NTU>10&Depth_m<0.3,NA,Turbidity_NTU)) #Sediment got mixed up in early 2022

max(df.final$Date[year(df.final$Date)==2021])
df.final%>%
  filter(month(Date)>9,
         year(Date)==2021)%>%
  arrange(Date)
...
write.csv(df.final%>%filter(Depth_m == 0.1),"../../BVR drawdown/Data/CTD at 0.1m.csv",row.names = F)
...
for(i in 1:length(vars)){
  ...
  p = interp%>%
    ...
    geom_ribbon(data = ctd_depths, aes(x = Date, ymin= WaterLevel, ymax = max), fill = "white",color = "black",lwd=.2)+
