...
atten_k = ctd%>%
  ...
  summarize(I0 = unique(I0),
            k = coef(lm(I(log(PAR_umolm2s)-logI0)~ 0 + Depth_m)),
            r2 = summary(lm(I(log(PAR_umolm2s)-logI0)~ 0 + Depth_m))$r.squared)%>%
  filter(r2>0.9)
...
data = ctd%>%filter(Date=="2018-11-19 00:00:00	")
i0 = data%>%
  filter(Depth_m<0)%>%
  summarize(mean(PAR_umolm2s))
logI0 = c(log(i0)$'mean(PAR_umolm2s)')
lm1 = lm(I(log(PAR_umolm2s)-logI0)~ 0 + Depth_m, data = data%>%filter(Depth_m>.1))
summary(lm1)
slope = coef(lm1)
summary(lm1)$r.squared

data%>%
  ggplot(aes(x = Depth_m, y = log(PAR_umolm2s)))+
  geom_point()+
  geom_abline(intercept = logI0, slope = slope)
