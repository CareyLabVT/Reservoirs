fcr_data_wrangling <- function(site){
  ...
  write_csv(ctd, paste0("../CTD_season_csvs/CTD_notmatlab_ready_2021_", site, ".csv"))
  ...
  write_csv(ctd_matlab, paste0("../CTD_season_csvs/CTD_matlab_ready_2021_", site, ".csv"), col_names = F)
  ...
  write_csv(ctd_new, paste0("../CTD_season_csvs/ctd_short_for_GLM_AED",site,"_2021.csv"))
...
ctd_vs_catwalk <- function(on,off,site,startDate = "2021-01-01 12:00:00"){
  ...
  if(site == "fcr50"){
    ...
    cat_sum_19 <- cat %>% filter(TIMESTAMP >= startDate) %>%
      select(TIMESTAMP, doobs_1, doobs_6, doobs_13,EXO_depth) %>%
      filter(doobs_1 != "NAN") %>%
      filter(doobs_6 != "NAN") %>%
      filter(doobs_13 != "NAN") %>%
      filter(TIMESTAMP != "NAN") %>%
    ...
    ctd_4.0 <- ctd_new %>% 
      group_by(Date)%>%
      filter(abs(Depth_m-4)==min(abs(Depth_m-4)))
    ...
    points(ctd_1.0$Date, ctd_1.0$DO_mgL, type = "p", pch = 21, col = "black", bg = "black", cex = 2, lwd = 2)
    points(ctd_4.0$Date, ctd_4.0$DO_mgL, type = "p", pch = 21, col = "black", bg = "blue", cex = 2, lwd = 2)
    points(ctd_11.0$Date, ctd_11.0$DO_mgL, type = "p", pch = 21, col = "black", bg = "magenta", cex = 2, lwd = 2)
    abline(v=on, lwd = 1.5)
    abline(v=off,lty=2, lwd = 1.5)
    legend("topright", legend=c("1.5 m", "4 m", "11 m"), pch = 21, pt.bg = c("black","blue","magenta"))
    dev.off()
    ...
    points(ctd_1.0$Date, ctd_1.0$Temp_C, type = "p", pch = 21, col = "black", bg = "black", cex = 2, lwd = 2)
    points(ctd_4.0$Date, ctd_4.0$Temp_C, type = "p", pch = 21, col = "black", bg = "blue", cex = 2, lwd = 2)
    points(ctd_11.0$Date, ctd_11.0$Temp_C, type = "p", pch = 21, col = "black", bg = "magenta", cex = 2, lwd = 2)
    abline(v=on, lwd = 1.5)
    abline(v=off,lty=2, lwd = 1.5)
    legend("topright", legend=c("1.5 m", "4 m", "11 m"), pch = 21, pt.bg = c("black","blue","magenta"))
    dev.off()
