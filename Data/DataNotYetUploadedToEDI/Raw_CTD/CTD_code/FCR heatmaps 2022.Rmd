...
files <- files[grepl("fcr",files)] #That are from fcr
...
write_csv(ctd, "../CTD_season_csvs/CTD_FCR_2022.csv")
```
  ...
  for(site in Sites){
    fcr<- df.final%>%
      ...
      filter(Site == site,
             Reservoir == "FCR",
             !is.na(get(var)))
    fcr_interp <- interp2xyz(interp(fcr$Date,fcr$Depth_m,fcr[[var]],xo = seq(min(fcr$Date), max(fcr$Date), .1),yo = seq(min(fcr$Depth_m), max(fcr$Depth_m), by = .01), duplicate = "mean"),data.frame = T)
    interp = interp%>%
      full_join(fcr_interp%>%mutate(Site=site))
  }
  interp = interp%>%
    filter(!is.na(Site))
  
  #Heatmap
  jpeg(paste0("../2022_BVR_drawdown_figs/FCR_",var,"_heatmap_2022.jpeg"),width = 8, height = 4, units = "in", res = 300)
  p = interp%>%
    ...
    geom_point(data = fcr, aes(x = Date, y = min(Depth_m)), shape = 25, fill = "black", color = "white", size  =2)+
...
for(var in vars){
  ...
  p = interp%>%
    ...
    labs(x = "", y = "Depth (m)")+
    scale_fill_gradientn(colours = if(var%in%c("DO_mgL","DO_pSat","ORP_mV")){rev(blue2green2red(100))}else{blue2green2red(100)}, na.value="gray", name = var)+
    geom_point(data = fcr, aes(x = Date, y = max(Depth_m)), shape = 25, fill = "black", color = "white", size  =2)+
    scale_x_date(expand = c(0,0))+
    theme(panel.border = element_rect(fill = NA))+
    facet_grid(rows= vars(Site),
               cols = vars(Year),
