...
This section graphs CO2 and CH4 and adds flags for missing data
```{r}
...
ghgs_reps <- ghgs_reps %>% 
  mutate(co2_diff = round((abs(co2_umolL_rep1-co2_umolL_rep2)/((co2_umolL_rep1+co2_umolL_rep2)/2))*100))
...
# If sample is BD (=0) then flag as 5 (below detection)
ghgs_comb_2 <- ghgs_comb_2 %>% 
  mutate(flag_co2 = ifelse(co2_umolL == 0 & is.na(ghgs_comb_2$flag_co2),5,
                           ifelse(co2_umolL == 0 & flag_co2 == 3, 35,
                           ghgs_comb_2$flag_co2))) %>% 
  mutate(flag_ch4 = ifelse(ch4_umolL == 0 & is.na(ghgs_comb_2$flag_ch4),5,
                           ifelse(ch4_umolL == 0 & flag_ch4 == 3, 35,
