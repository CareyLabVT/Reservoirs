...
NOTES
1. Need to revise the flora heatmap to actually work
2. Then knit so a reviewer can look at it
...
pacman::p_load(tidyverse, lubridate, gsheet, plotly, akima)
```
...
df.final = bind_rows(df.final, fp_layer)
  ...
  interp <- interp(x=fp_new$DOY, y = fp_new$Depth_m, z = fp_new$TotalConc_ugL,
...
interp_green <- interp2xyz(interp_green, data.frame=T)
  ...
  fig <- ggplot(data = fp_new, aes(x = DateTime, y = Depth_m, color = TotalConc_ugL))+
    geom_point()+
    theme_classic()+
    ggtitle(fig_title)
...
fig1
fig2 <- flora_heatmap(fp_data = current_df, reservoir = "BVR", year = 2023, site = 50, z = "TotalConc_ugL")
fig2
fig3 <- flora_heatmap(fp_data = current_df, reservoir = "CCR", year = 2023, site = 50, z = "TotalConc_ugL")
fig3
```

```{r visualization of an historic year at a glance}
fig4 <- flora_heatmap(fp_data = current_df, reservoir = "FCR", year = 2016, site = 50, z = "TotalConc_ugL")
fig4
fig5 <- flora_heatmap(fp_data = current_df, reservoir = "BVR", year = 2016, site = 50, z = "TotalConc_ugL")
fig5
fig6 <- flora_heatmap(fp_data = current_df, reservoir = "CCR", year = 2016, site = 50, z = "TotalConc_ugL")
fig6
fig7 <- flora_heatmap(fp_data = current_df, reservoir = "SHR", year = 2016, site = 50, z = "TotalConc_ugL")
fig7
fig8 <- flora_heatmap(fp_data = current_df, reservoir = "GWR", year = 2016, site = 50, z = "TotalConc_ugL")
fig8
```
