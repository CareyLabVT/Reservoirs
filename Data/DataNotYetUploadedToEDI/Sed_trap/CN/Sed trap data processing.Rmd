...
cn_processed = cn_raw%>%
  ...
  mutate(Reservoir = ifelse(substring(Name,1,1)%in%c("f","F"),"FCR",NA),
         ...
         filter1_ID = ifelse(grepl("_",cut), #First, standardize filter columns
                             sub("_.+","",cut),
                             paste0("r1",cut)),
         filter2_ID = ifelse(grepl("_",cut),
                             sub(".+_","",cut),
                             paste0("r2",cut)),
         filter1_ID = paste0(substr(Reservoir,1,1),"_sed_",
                             Depth_m,"m_R",substr(filter1_ID,2,2),"_F",substr(filter1_ID,4,4),"_",
                             format(Date_sampled,"%d%b%y")),
         filter2_ID = paste0(substr(Reservoir,1,1),"_sed_",
                             Depth_m,"m_R",substr(filter2_ID,2,2),"_F",substr(filter2_ID,4,4),"_",
                             format(Date_sampled,"%d%b%y")))%>%
