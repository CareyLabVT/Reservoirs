...
wd <- setwd("~/Desktop/github/Reservoirs/Data/DataNotYetUploadedToEDI/UGGA/UGGA_Raw/2019/TextFiles")
...
#flux_lgr_2 <- LoadLGR(file ="./gga_2019-04-30_f0000.txt", #several peaks, no record
#                      time_format = "mdy_HMS")
...
Flux_output4 <- FluxCal(data = flux_lgr_4, # Dataframe loaded in
                        win = 2.5, # Window length shortened to match data
...
flux_output <- rbind(Flux_output4,
                     Flux_output5,
                     Flux_output7,
                     Flux_output9,
                     Flux_output10,
...
# look to see what reservoir the data came from
flux_all <- flux_all %>% 
