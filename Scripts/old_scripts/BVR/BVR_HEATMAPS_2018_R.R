...
ctd <- read_csv("CTD_Appended_bvr50_2018.csv")
...
ctd33<-ctd %>% group_by(Date) %>% slice(which.min(abs(as.numeric(Depth_m) - 9.9)))
ctd34<-ctd %>% group_by(Date) %>% slice(which.min(abs(as.numeric(Depth_m) - 10.2)))
...
pdf("BVR_CTD_2018.pdf", width=10, height=30)
